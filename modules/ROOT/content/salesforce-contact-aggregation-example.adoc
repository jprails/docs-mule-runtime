...
This example application uses the https://mulesoft.github.io/salesforce-connector/[Salesforce connector] to query contacts from two separate Salesforce instances and determine which contacts are present in both instances. The section immediately below describes the functions of each of the flows in the application. The section further below describes the configuration of the Salesforce Global Connector Configuration through which the Mule application establishes a connection to Salesforce.

Beyond this example, access https://mulesoft.github.io/salesforce-connector/[introductory] and https://mulesoft.github.io/salesforce-connector/[reference] documentation for the Salesforce Connector.

== Prerequisites


This document assumes that you are familiar with Mule, the xref:5@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:5@studio::datasense/datasense.adoc[DataSense], and the xref:5@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper transformer]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:flows/flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
...
Invoked by the *triggerFlow* (configured in a separate mflow file in the application), the mainFlow begins processing the message triggerFlow accepted via HTTP request. The mainFlow consists of four *Flow Ref* scopes, which successively call the four subflows in the application. The flow also references a xref:exceptions/catch-exception-strategy.adoc[catch exception strategy] to handle exceptions thrown. At runtime, the mainFlow begins by calling the *gatherDataFlow*, explained below the example.
...
This flow contains two xref:scopes/message-enricher.adoc[message enricher] scopes with a Salesforce connector in each. Each Salesforce connector connects to a Salesforce instance to retrieve all fields of type *Contact*. The message enricher for each connector stores this information in two xref:mule-message-structure.adoc[flow variables], one for each Salesforce organization.
...
Each Salesforce connector references a Salesforce xref:global-elements.adoc[global element], which is configured with the connection parameters (user ID, password, etc.) for each Salesforce organization. To see the configuration for each global element, see the <<Salesforce Global Elements>> section.
...
Next, a xref:5@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps the Java object to a CSV file which the flow then transforms to a string. The flow passes the message, now a string of duplicated email addresses back to the mainFlow. The mainFlow proceeds to send the message to the *outboundFlow* (configured in a separate mflow file in the application) to email the results to a pre-defined address.
...
* Access the introductory material for the https://mulesoft.github.io/salesforce-connector/[Salesforce Connector].

* Access https://mulesoft.github.io/salesforce-connector/[full reference documentation] for the Salesforce Connector.

* Learn more about xref:mule-message-structure.adoc[flow variables].

* Learn more about xref:flows/flows-and-subflows.adoc[Flows and Subflows].

* Learn more about xref:3.7@connectors::index.adoc[Anypoint Connectors] in general.