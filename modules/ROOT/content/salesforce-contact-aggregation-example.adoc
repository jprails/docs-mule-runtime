...
Beyond this example, access https://mulesoft.github.io/salesforce-connector/[introductory] and link:/mule-user-guide/v/3.8/salesforce-connector-reference[reference] documentation for the Salesforce Connector.

== Prerequisites

This document assumes that you are familiar with Mule, the xref:6@studio::index.adoc[Anypoint Studio interface], xref:global-elements.adoc[Global Elements], xref:6@studio::datasense/datasense.adoc[DataSense], and the xref:dataweave/index.adoc[DataWeave] or its predecessor, xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Datamapper]. Further, it is assumed that you are familiar with Salesforce and have a http://developer.force.com/[developerforce] account. Learn more about xref:flows/flows-and-subflows.adoc[flows and subflows] to gain more insight into the design of this example.
...
Next, a xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper transformer] maps the Java object to a CSV file which the flow then transforms to a string. The flow passes the message, now a string of duplicated email addresses back to the mainFlow. The mainFlow proceeds to send the message to the *outboundFlow* (configured in a separate mflow file in the application) to email the results to a pre-defined address.
...
* Learn more about xref:3.8@connectors::index.adoc[Anypoint Connectors] in general.