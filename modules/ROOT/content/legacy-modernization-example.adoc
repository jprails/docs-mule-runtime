...
For more information on mapping nested elements, refer to xref:5@studio::mapping-elements-inside-lists.adoc[Mapping Elements Inside Lists].
====
...
* Learn more about the xref:components/cxf-component-reference.adoc[CXF Component] in Studio.
* Learn more about the xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper transformer] in Studio.
* Examine other xref:mule-examples.adoc[Mule application examples], particularly the SaaS Integration example which uses DataMapper and xref:5@studio::datasense/datasense.adoc[DataSense] to intelligently connect an application to Salesforce.