...
:page-aliases: using-maven-with-mule.adoc
...
* xref:5@studio::maven/building-a-mule-application-with-maven-in-studio.adoc[Building a Mule application with Maven in Studio]
* xref:5@studio::maven/enabling-maven-support-for-a-studio-project.adoc[Enabling Maven support for an existing Studio project]
* xref:5@studio::maven/importing-a-maven-project-into-studio.adoc[Importing a Maven project into Studio]

*In Mule* *ESB:*

Maven-Mule ESB functionality is provided by two Mule plugins for Maven.

* xref:3.7@maven/maven-tools-for-mule-esb.adoc[Maven tools for Mule ESB] is a plugin that provides archetypes for building regular Mule applications, allowing you to develop Mule applications from within Maven
* The xref:3.7@maven/mule-esb-plugin-for-maven.adoc[muleesb-maven-plugin] allows you to control Mule ESB instances from within a Maven environment
...
* xref:5@studio::maven/using-maven-in-anypoint-studio.adoc[Using Maven in Anypoint Studio]
** xref:5@studio::maven/maven-support-in-anypoint-studio.adoc[Maven Support in Anypoint Studio]
** xref:5@studio::maven/building-a-mule-application-with-maven-in-studio.adoc[Building a Mule Application with Maven in Studio]
** xref:5@studio::maven/enabling-maven-support-for-a-studio-project.adoc[Enabling Maven Support for a Studio Project]
** xref:5@studio::maven/importing-a-maven-project-into-studio.adoc[Importing a Maven Project into Studio]
* xref:3.7@maven/using-maven-in-mule-esb.adoc[Using Maven in Mule ESB]
** xref:3.6@maven/configuring-maven-to-work-with-mule-esb.adoc[Configuring Maven to Work with Mule ESB]
** xref:3.7@maven/maven-tools-for-mule-esb.adoc[Maven Tools for Mule ESB]
** xref:3.7@maven/mule-esb-plugin-for-maven.adoc[Mule ESB Plugin For Maven]
* xref:3.7@maven/maven-reference.adoc[Maven Reference]
...
* **Build and run a new Maven project in Mule Studio.** If you're starting from scratch and you want to use Maven to build your project, begin by creating a new Maven-based project in Studio. You can then run the project with Maven from the command line, or run it directly from Studio. See xref:5@studio::maven/building-a-mule-application-with-maven-in-studio.adoc[Building a Mule Application with Maven in Studio] for a step-by-step guide demonstrating how to create, manage, and run a Mule project with Maven.

* *Create and run a new Maven in Mule ESB.* If you want to use Maven to create your project for Mule ESB (outside Studio), use *Maven Tools for Mule ESB*. This is a Mule plugin that makes it extremely easy to create a Mule application by running Maven with a few simlpe parameters. You need to add dependencies for any xref:3.7@connectors::index.adoc[Anypoint Connectors] that you want to use in your project and, if you are an Enterprise customer, configure your settings.xml file to point to the MuleSoft Enterprise repository. To enable access, click *Help* > *Install New Software* and in *Work with*, click *Anypoint Addons Update Site* > *Maven Tools for Mule*. Click *Next* and follow the install prompts:
+
image::maven/maventoolsformule.png[MavenToolsForMule]

* **Import an existing Maven project into Studio.** If you already have a Maven project you want to work on further with Studio, use the built-in functionality in Studio to import the pom.xml file using the `studio:studio` goal. Access xref:5@studio::maven/importing-a-maven-project-into-studio.adoc[Importing a Maven Project into Studio] for details.

* **Mavenize an existing Studio project.** If you have built a project in Studio and want to convert it to a Maven project, you can use the built-in support in Studio to "Mavenize" the project. Access xref:5@studio::maven/enabling-maven-support-for-a-studio-project.adoc[Enabling Maven Support for a Studio Project] for details.
...
+
image::maven/snapshot.png[]

== See Also

* Learn more about xref:5@studio::maven/maven-support-in-anypoint-studio.adoc[adjusting your Studio Preferences to work with Maven].
* Learn more about xref:5@studio::maven/importing-a-maven-project-into-studio.adoc[importing Maven projects into Studio] or xref:5@studio::maven/enabling-maven-support-for-a-studio-project.adoc["mavenizing" existing Studio projects].
* Using Maven outside of Studio? Refer to these instructions for xref:3.6@maven/configuring-maven-to-work-with-mule-esb.adoc[configuring your Maven installation] to point to the necessary repositories.
* Access the xref:3.7@maven/maven-reference.adoc[Maven reference documentation].