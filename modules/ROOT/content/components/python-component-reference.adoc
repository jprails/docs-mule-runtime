...
:page-aliases: python-component-reference.adoc
...
[TIP]
Placing a Python Component is actually the same as placing a xref:components/script-component-reference.adoc[Script component] and then selecting the Python engine.
...
image::components/python-general.png[python_general]
...
Use the Advanced tab to optionally configure interceptors and, depending on the interceptor, enter Spring values.You also have the option to specify script properties, which are key/value pairs used to alter or change properties in the script. See the xref:components/script-component-reference.adoc[Script Component Reference] page for information on using the advanced tab.

image::components/python-advanced.png[python_advanced]
...
When deploying to a customer managed Mule runtime (one that's deployed on-premises), then you can set the `python.path` property directly in the `wrapper.conf` file. See xref:configuring-properties.adoc#system-properties[system properties] for a better understanding of how to set it.
...
* Refer to the generic xref:components/script-component-reference.adoc[Script Component Reference] to see how a script works with one-way and request-response connectors.
* For more information see the Mule ESB page xref:modules/scripting-module-reference.adoc[Scripting Module].