...
[TIP]
Placing a Python Component is actually the same as placing a xref:3.5@components/script-component-reference.adoc[Script component] and then selecting the Python engine.
...
image::components/pythonpropgen.png[PythonPropGen]
...
Use the *Advanced* tab to optionally configure interceptors and (depending on the interceptor) enter Spring values. You also have the option to specify script properties, which are key/value pairs used to alter or change properties in the script. See the xref:3.5@components/script-component-reference.adoc[Script Component Reference] page for information on using the advanced tab.
...
The example below uses the https://en.wikipedia.org/wiki/Change-making_problem[“Greedy Coin Changer” algorithm] to transform one currency into another.  It uses some transformers to transform the input data and then applies a Python algorithm to convert the currency.
...
* Refer to the generic xref:3.4@components/script-component-reference.adoc[Script Component Reference] to see how a script works with one-way and request-response endpoints.

* For more information see the Mule ESB page xref:3.4@modules/scripting-module-reference.adoc[Scripting Module].