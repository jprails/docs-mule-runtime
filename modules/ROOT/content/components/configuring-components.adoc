...
:page-aliases: configuring-components.adoc

[%autowidth.spread]
|===
|[xref:components/configuring-components.adoc[Simple Components]] |[xref:components/configuring-components.adoc[Java Components]] |[xref:components/configuring-components.adoc[Other Components]] |[xref:components/configuring-components.adoc[Customizing Behavior with Interceptors]] |[xref:components/configuring-components.adoc[Lifecycle]]
|===

Components contain the business logic for working with the message passed through Mule ESB. A component can be any type of object, including a xref:spring-integration/using-spring-beans-as-flow-components.adoc[Spring bean], xref:components/configuring-java-components.adoc[POJO], xref:modules/scripting-module-reference.adoc[script], xref:api-usage/using-web-services.adoc[web service]. or REST call.
...
For detailed information on the elements you configure for components, see xref:components/component-configuration-reference.adoc[Component Configuration Reference]
...
|*<test:component/>* |Configures the `MuleFunctionalTestComponent`, which allows more complex testing scenarios to be created. For more information, see xref:testing/functional-testing.adoc[Functional Testing].
|===
...
Java components can be configured quickly and easily by simply specifying the component implementation class name on the `<component>` or `<pooled-component>` element. The `<pooled-component>` element allows you to establish a pooling profile for the flow (see xref:administer/tuning-performance.adoc[Tuning Performance]). In both cases, the `PrototypeObjectFactory` will be used by default and a new object instance will be created for each request of (for pooled components) for each new object in the pool.
...
The explicit syntax is required instead of the shortcut `<component class>` syntax if you add interceptor to the component. For further configuration options and information on the default settings that are applied, see xref:components/configuring-java-components.adoc[Configuring Java Components].

== Other Components

These are several other components available that allow you to use different technologies such as xref:api-usage/using-web-services.adoc[web services] for your components. These components are often included as part of xref:transports/transports-reference.adoc[transport] or xref:modules/modules-reference.adoc[modules].

[%header%autowidth.spread]
|===
|Configuration |Description
|*<http:rest-service-component/>* |Proxies a remote call to a xref:components/rest-component-reference.adoc[REST-style web service].
|*<cxf:wrapper-component/>* |Proxies a remote call to a web service using xref:modules/building-web-services-with-cxf.adoc[CXF].
|*<script:component/>* |Configures a JSR-223 xref:modules/scripting-module-reference.adoc[script] for the component.
|===

== Customizing Behavior with Interceptors

Mule interceptors are useful for attaching behaviors to multiple components. The interceptor pattern is often referred to as practical AOP (Aspect Oriented Programming), as it allows the developer to intercept processing on an object and potentially alter the processing and outcome. For complete information, see xref:components/using-interceptors.adoc[Using Interceptors].
