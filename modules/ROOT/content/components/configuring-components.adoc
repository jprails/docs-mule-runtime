...
|<test:component/> |Configures the Mule `FunctionalTestComponent`, which allows more complex testing scenarios to be created. For more information, see xref:testing/functional-testing.adoc[Functional Testing].
|===
...
The explicit syntax is required instead of the shortcut `<component class>` syntax if you add interceptors to the component. For further configuration options and information on the default settings that are applied, see xref:components/configuring-java-components.adoc[Configuring Java Components].

== Other Components

These are several other components available that allow you to use different technologies such as xref:api-usage/using-web-services.adoc[web services] for your components. These components are often included as part of xref:transports/transports-reference.adoc[transports] or xref:modules/modules-reference.adoc[modules].
