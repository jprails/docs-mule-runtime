...
This component evaluates an xref:3.4@mel/mule-expression-language-mel.adoc[expression]. It accepts either a user-defined expression or a reference to a file.

*Minimum requirement:* The expression or file to evaluate.

== Configuration

Unlike the xref:3.4@transformers/expression-transformer-reference.adoc[Expression Transformer], the expression component does not replace the message payload with the results of expression evaluation. See the *Implementing Business Logic* subsection in xref:3.4@mel/mule-expression-language-mel.adoc[Mule Expression Language MEL] for an example illustrating the use of an expression component.

=== General Tab

image::components/props.png[]
