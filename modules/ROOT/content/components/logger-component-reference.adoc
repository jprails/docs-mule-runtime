...
:page-aliases: logger-component-reference.adoc
...
By default, Mule Studio will not log messages at the DEBUG or TRACE level to the console unless you create and configure a xref:components/logger-component-reference.adoc[`log4j.properties` file] in `src/main/resources` to lower the log level. |
[source,text,linenums]
----
level="INFO"
----
...
String |Optionally specify a category name and configure it in the xref:components/logger-component-reference.adoc[`log4j.properties file`] to behave per your use case. For example, you can route log messages based on category or set log levels based on category. |
[source,text,linenums]
----
...
|Category |_Optional_. Specify what categories to route log entries according to business needs. Configure the categories in your xref:components/logger-component-reference.adoc[`log4j.properties` file]
...
* Check the xref:mel/mule-expression-language-mel.adoc[Mule Expession Language] reference for more ideas on how to format helpful logger messages.