...
:page-aliases: http-response-builder.adoc
...
When the xref:3.7@connectors::http-listener-connector.adoc[HTTP Connector works as a listener], you can configure a status code and a status reason on the connector itself, separately for both success and failure scenarios. To generate a response from an HTTP template, use the *xref:transformers/parse-template-reference.adoc[Parse Template]* element.

See xref:3.7@connectors::http-connector.adoc[HTTP Connector] for information about the 3.6 and later operation-based HTTP connector. For information on the deprecated HTTP connector, see xref:3.7@connectors::http-connector-deprecated.adoc[HTTP Connector - Deprecated].
...
image::components/ex-1-2.png[ex+1+2]
...
image::components/cookies-headers.png[cookies+headers]
...
+
image::components/ex2-flow-1.png[ex2+flow-1]
...
* Read about the xref:3.7@connectors::http-connector.adoc[HTTP Connector]
