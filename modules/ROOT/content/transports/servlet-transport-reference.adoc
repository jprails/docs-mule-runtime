...
https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/transport/servlet/ServletConnector.html[Mule Servlet Transport Javadoc]
...
For more information about using Mule with servlet containers, see xref:deploy/embedding-mule-in-a-java-application-or-webapp.adoc[Embedding Mule in a Java Application or Webapp].
...
|===
|Name |Description
|servletUrl |The real URL on which the servlet container is bound. If this is not set, the WSDL may not be generated correctly when using CXF bound to a servlet endpoint.

*Type*: string +
*Required*: no +
*Default*: none
|useCachedHttpServletRequest |Whether to use a cached http servlet request.
...
Servlet endpoints accept the same attributes and child elements as HTTP endpoints. For more information, see xref:transports/file-transport-reference.adoc[FTP Transport Reference].
...
|===
|Name |Description
|path |The servlet path to bind the service to.
