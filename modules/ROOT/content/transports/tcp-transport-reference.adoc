...
:page-aliases: tcp-transport-reference.adoc
...
* The xref:transports/ssl-and-tls-transports-reference.adoc[SSL and TLS Transports], which use TCP with socket-level security.
...
|Doc |https://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/transport/multicast/package-summary.html[JavaDoc] +
...
|Inbound |image:transports/check.png[] |Whether the transport can receive inbound events and can be used for an inbound endpoint.
|Outbound |image:transports/error.png[check] |Whether the transport can produce outbound events and be used with an outbound endpoint.
|Request |image:transports/check.png[] |Whether this endpoint can be queried directly with a request call (via MuleClient or the EventContext)
|Transactions |image:transports/error.png[] |Whether transactions are supported by the transport. Transports that support transactions can be configured in either local or distributed two-phase commit (XA) transaction.
|Streaming |image:transports/error.png[] |Whether this transport can process messages that come in on an input stream. This allows for very efficient processing of large data. For more information, see Streaming.
|Retries |image:transports/error.png[] |Whether this transport supports retry policies. Note that all transports can be configured with Retry policies, but only the ones marked here are officially supported by MuleSoft.
...
* Are you communicating with a flow always located in the same Mule application instance? If so, consider use the xref:transports/vm-transport-reference.adoc[VM transport].

* Is it important that messages be persisted until they can be processed? If so, consider using a persistent transport like xref:transports/jms-transport-reference.adoc[JMS] or xref:transports/file-transport-reference.adoc[File].

* Are there advantages to a higher-level protocol built on top of TCP, for instance, the request-response features of xref:transports/deprecated-http-transport-reference.adoc[HTTP], or the store-and-forward features of xref:transports/email-transport-reference.adoc[Email]? If so, use the transport for that protocol instead.

* Is performance the primary concern and it is not important that messages be delivered in the proper order or that the sender is notified if any are lost? If so, use the lighter-weight xref:transports/udp-transport-reference.adoc[UDP transport] instead.

* Should messages be secured? If so, use the xref:transports/ssl-and-tls-transports-reference.adoc[SSL transport].

* If you get this far, TCP is a good candidate.

As shown in the examples below, the TCP transport can be used for:
...
* To create a TCP server that accepts incoming connections, declare an inbound tcp endpoint with a http://tcpconnector[tcp:connector]. This creates a TCP server socket that will read requests from and optionally write responses to client sockets.
* To poll from a TCP server, declare an inbound tcp endpoint with a http://tcppolling-connector[tcp:polling-connector]. This creates a TCP client socket that will read requests from and optionally write responses to the server socket.
* To write to a TCP server, create an outbound endpoint with a http://tcpconnector[tcp:connector]. This creates a TCP client socket that will write requests to and optionally read responses from a server socket.
...
To act as a server that listens for and accepts TCP connections from clients, create a simple TCP connector that inbound endpoints uses:
...
To act as a client that repeatedly opens connections to a TCP server and reads data from it, create a polling connector that inbound endpoints uses:
...
To send messages on a TCP connection, create a simple TCP connector that outbound endpoints uses:
...
|sendTcpNoDelay |boolean |no |  |If set, transmitted data is not collected together for greater efficiency but sent immediately.
|reuseAddress |boolean |no |  |If set (the default), SO_REUSEADDRESS is set on server sockets before binding. This helps reduce "address already in use" errors when a socket is re-used.
|connectionTimeout |integer |no | None |Number of milliseconds to wait until an outbound connection to a remote server is successfully created.
|clientSoTimeout |integer |no |  |This sets the SO_TIMEOUT value when the socket is used as a client. Reading from the socket will block for up to this long (in milliseconds) before the read fails. A value of 0 (the default) causes the read to wait indefinitely (if no data arrives).
|serverSoTimeout |integer |no |  |This sets the SO_TIMEOUT value when the socket is used as a server. Reading from the socket will block for up to this long (in milliseconds) before the read fails. A value of 0 (the default) causes the read to wait indefinitely (if no data arrives).
|socketSoLinger |integer |no |  |This sets the SO_LINGER value. This is related to how long (in milliseconds) the socket will take to close so that any remaining data is transmitted correctly.
|keepSendSocketOpen |boolean |no |  |If set, the socket is not closed after sending a message. This attribute only applies when sending data over a socket (Client).
...
|rethrowExceptionOnRead |boolean |no |  |Rethrow the exception if read fails
|payloadOnly |boolean |yes |  |Sends only the payload, not the entire Mule message object or its properties. This defaults to true when the protocol is not specified explicitly (when the safe protocol is used).
|maxMessageLength |integer |no |  |An optional maximum length for the number of bytes in a single message. Messages larger than this trigger an error in the receiver, but it gives an assurance that no out-of-memory error will occur.
|===
...
https://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/transport/tcp/package-summary.html[TCP]
