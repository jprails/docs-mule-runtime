...
A transport is responsible for carrying messages from application to application in the Mule ESB framework. In EIP terms, transports are used to implement message channels and provide connectivity to an underlying data source or message channel in a consistent way. For example, the xref:http-transport-reference.adoc[HTTP transport] handles messages sent via the HTTP protocol, whereas the xref:3.6@transports/file-transport-reference.adoc[File transport] reads and writes files placed on the server's file system.
