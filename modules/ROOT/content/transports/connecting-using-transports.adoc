...
A transport is responsible for carrying messages from application to application in the Mule ESB framework. In EIP terms, transports are used to implement message channels and provide connectivity to an underlying data source or message channel in a consistent way. For example, the xref:3.7@transports/deprecated-http-transport-reference.adoc[HTTP transport] handles messages sent via the HTTP protocol, whereas the xref:3.6@transports/file-transport-reference.adoc[File transport] reads and writes files placed on the server's file system.
...
Transports are configured via endpoints in Mule. See xref:3.6@endpoints/endpoint-configuration-reference.adoc[Endpoint Configuration Reference] for an explanation on how this is done.

A list of available transports and the reference topic including example configuration for each specific transport bundled with Mule can be found in the xref:3.6@transports/transports-reference.adoc[Transport Configuration Reference].

image::transports/transports.png[]

== See Also

xref:3.6@transports/transports-reference.adoc[Available Transports]