...
:page-aliases: activemq-integration.adoc
...
Mule initializes the ActiveMQ connector with a default instance of ActiveMQ connection factory and establishes a TCP connection to the remote standalone broker running on local host and listening on port 61616.
...
|numberOfConsumers |The number of concurrent consumers to use to receive JMS messages. (Note: If you use this attribute, you should not configure the 'numberOfConcurrentTransactedReceivers', which has the same effect.)
