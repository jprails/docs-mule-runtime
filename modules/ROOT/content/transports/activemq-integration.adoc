...
The usage of https://repository.apache.org/content/repositories/releases/org/apache/activemq/activemq-all/[activemq-all-x.x.x.jar] creates conflicts with other dependencies, most notably with log4j, so only add the JAR files required by the deployment mode you have chosen for ActiveMQ:
