...
The Jetty transport provides support for exposing services over HTTP by embedding a light-weight Jetty server. The xref:transports/jetty-ssl-transport.adoc[Jetty SSL Transport] works the same way but over SSL. You can only define inbound endpoints with this transport.
