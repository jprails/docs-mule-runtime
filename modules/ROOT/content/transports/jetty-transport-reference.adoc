...
Jetty endpoints are configured the same way as xref:transports/file-transport-reference.adoc[FTP endpoints]. Note that the Jetty transport can only be used for inbound endpoints.

For example:

[source,xml,linenums]
----
include::{samplesdir}/jetty-transport-reference.xml[]
----