...
Mule also supports the SFTP protocol for secure file transfer. As of Mule 3, the xref:transports/sftp-transport-reference.adoc[SFTP Transport] is included in the Mule distribution.
...
|http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_ftp/namespace-overview.html[javaDocSchemaDoc]
 |image:transports/check.png[] |image:transports/check.png[] |image:transports/check.png[] |- |image:transports/check.png[] |image:transports/check.png[] |one-way |one-way |org.mule.transport:mule-transport-ftp
|===
 ...
 *Maven Artifact* - The group name a artifact name for this transport in linkhttp://maven.apache.org/[Maven]
====
...
If streaming is not enabled on the FTP connector, Mule will attempt to read a file it picks up from the FTP server into a `byte[]` to be used as the payload of the `MuleMessage`. This behavior can cause trouble if large files need to be processed.
...
Which allows use of xref:non-mel-expressions/non-mel-expressions-configuration-reference.adoc[arbitrary expressions] to compose the filename to use when storing files on the FTP server.
