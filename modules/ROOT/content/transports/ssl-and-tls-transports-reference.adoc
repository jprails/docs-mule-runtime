...
[WARNING]
This page refers to using SSL and TLS together with the old endpoint based xref:3.7@connectors::http-connector.adoc[HTTP connector]. For using TLS with the new HTTP Connector and the Web Service Consumer connector, see xref:3.7@connectors::tls-configuration.adoc[TLS Configuration].
...
|===
|Name |Notes
|class |See xref:transports/tcp-transport-reference.adoc[TCP Transport Reference] for an example of writing a custom protocol
