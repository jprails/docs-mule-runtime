...
[WARNING]
This page refers to using SSL and TLS together with the old endpoint based xref:3.6@connectors::http-connector.adoc[HTTP connector]. For using TLS with the new HTTP Connector and the xref:3.6@connectors::web-service-consumer.adoc[Web Service Consumer] connector, see xref:3.6@connectors::tls-configuration.adoc[TLS].

The following Mule transports provide access to TCP connections:

* The xref:transports/tcp-transport-reference.adoc[TCP Transport], which uses the basic TCP transport.
...
* Communicating with an external service that uses low-level unsecured TCP connections? If so, use the xref:transports/tcp-transport-reference.adoc[TCP] protocol.
