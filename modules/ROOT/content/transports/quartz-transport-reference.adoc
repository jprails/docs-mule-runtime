...
:page-aliases: quartz-transport-reference.adoc
:keywords: quartz

[TIP]
When scheduling tasks, it's recommended that you instead resort to the xref:scopes/poll-reference.adoc[Poll Scope]
...
If you are not familiar with cron syntax, don't be intimidated! It's reasonably easy to learn, here is a http://www.quartz-scheduler.org/documentation/quartz-2.1.x/tutorials/crontrigger.html[good tutorial], or you can even leverage online tools that can build cron expressions for you.
...
* See the xref:3.7@connectors::quartz-connector.adoc[Quartz Connector] for details on setting the properties for a Quartz endpoint in Studio's visual editor.
