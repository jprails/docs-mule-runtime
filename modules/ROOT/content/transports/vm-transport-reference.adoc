...
The In Memory transport can be used for intra-JVM communication between Mule xref:flows/using-flows-for-service-orchestration.adoc[Flows] and/or xref:using-mule-services.adoc[Services]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues.
...
If you need to invoke a VM endpoint from https://www.mulesoft.org/docs/site/current3/apidocs/org/mule/module/client/MuleClient.html[Mule Client], use an xref:endpoints/mule-endpoint-uris.adoc[Endpoint URI]. The format of an endpoint URI for VM is as follows:
