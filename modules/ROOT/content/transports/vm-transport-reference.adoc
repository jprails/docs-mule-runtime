...
The VM transport can be used for intra-JVM communication between Mule xref:flows/flows-and-subflows.adoc[flows]. This transport by default uses in-memory queues but can optionally be configured to use persistent queues. (Note, however, that VM file persistency does not work on clusters.)

[NOTE]
Note that VM queue persistence behaves differently on CloudHub. See the xref:cloudhub::cloudhub-fabric.adoc[CloudHub Fabric] documentation for more details.
