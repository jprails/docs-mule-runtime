...
To support the case where a JNDI registry has been configured to store the connection factory, the connector declaration should include the following parameters. This is the same as the regular xref:3.7@transports/jms-transport-reference.adoc[JMS transport].
...
The WMQ transport supports xref:3.6@mule-configuration/configuring-reconnection-strategies.adoc[retry policies] . You can configure the timeout value on the connector as follows:
