...
The AXIS and xref:modules/cxf-module-reference.adoc[CXF] transports provide WSDL connectors that can be used for invoking remote web services by obtaining the service WSDL. Mule creates a dynamic proxy for the service and then invokes it.

The Javadoc for the Axis WSDL connector is https://www.mulesoft.org/docs/site/current/apidocs/org/mule/transport/soap/axis/wsdl/package-summary.html[here], and for the CXF WSDL connector it's https://www.mulesoft.org/docs/site/current/apidocs/org/mule/transport/cxf/wsdl/package-summary.html[here].
...
This example demonstrates how to take multiple arguments from the console, invoke a web service, and print the output to the screen. It uses the http://www.webservicex.net/WCF/ServiceDetails.aspx?SID=18[Currency Convertor] web service on www.webservicex.net.
...
There are several ways to construct this object array, but the easiest way is to use the custom transformer https://www.mulesoft.org/docs/site/current/apidocs/org/mule/transformer/simple/StringToObjectArray.html[StringToObjectArrayTransformer], which translates a delimited String into an Object array. In the example below, you simply type in a String in the format of `<fromCurrency>,<toCurrency>`.
