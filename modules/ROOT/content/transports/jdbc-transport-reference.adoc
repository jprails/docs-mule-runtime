...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[SchemaDoc] |image:transports/check.png[] |image:transports/check.png[] |image:transports/check.png[] |image:transports/check.png[](local,XA) |image:transports/error.png[] |image:transports/check.png[] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
|===
...
Note that stored procedures are only supported on outbound endpoints. If you want to set up a service that calls a stored procedure at a regular interval, you can define a xref:transports/quartz-transport-reference.adoc[Quartz] inbound endpoint and then define the stored procedure call in the outbound endpoint. For information on using Quartz to trigger services, see the following http://blog.mulesoft.org/2009/06/using-quartz-to-trigger-a-service/[blog post].
...
The Javadoc for this transport can be found https://www.mulesoft.org/docs/site/current/apidocs/org/mule/transport/jdbc/package-summary.html[here]. Refer to the EE distribution for the enterprise version of the JDBC transport javadocs.
...
* Put your database connection and credential information in a separate properties file. This allows you to port your configuration file to different environments. See <<Example Configurations>> or the xref:jdbc-transport-example.adoc[JDBC Transport Example] for an example on how this is done
