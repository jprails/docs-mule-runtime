...
:page-aliases: jdbc-transport-reference.adoc
...
http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[SchemaDoc] |image:transports/check.png[(tick)] |image:transports/check.png[(tick)] |image:transports/check.png[(tick)] |image:transports/check.png[(tick)](local,XA) |image:transports/error.png[(error)] |image:transports/check.png[(tick)] |one-way, request-response |one-way |org.mule.transport:mule-transport-jdbc
|===
...
Note that stored procedures are only supported on outbound endpoints. If you want to set up a flow that calls a stored procedure at a regular interval, you can define a xref:transports/quartz-transport-reference.adoc[Quartz] inbound endpoint and then define the stored procedure call in the outbound endpoint. For information on using Quartz to trigger flows, see the following http://blog.mulesoft.org/2009/06/using-quartz-to-trigger-a-service/[blog post].
...
Stored procedures/functions can only be called on JDBC outbound endpoints. Once the values are returned from the database, they are put in a `java.util.HashMap` with key/value pairs. The keys are the parameter names, for example, "mules2", while the values are the Java data values (Integer, String, etc.). This hash map is the payload of MuleMessage either returned to the caller or sent to the next endpoint depending on the Mule configuration.
...
Transactions are supported on JDBC endpoints. See xref:transactions/transaction-management.adoc[Transaction Management] for details.
...
You can view the full schema for the JDBC transport http://www.mulesoft.org/docs/site/current3/schemadocs/namespaces/http_www_mulesoft_org_schema_mule_jdbc/namespace-overview.html[here]. The enterprise version of the schema docs is not available.
...
*Tip* +
 +
Use the http://jdbcderby-data-source[jdbc:derby-data-source] configuration element to configure Derby. If you use a regular bean, you may receive errors when undeploying or redeploying the application.
