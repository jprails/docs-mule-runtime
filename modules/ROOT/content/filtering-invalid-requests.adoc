...
This example adds a new filter to the existing 'core-concepts' project created in the xref:adding-business-logic-to-a-flow.adoc[Adding Business Logic to a Flow] example.
...
* xref:components/developing-components.adoc[Working with Components]

== Next Steps

Next: xref:transforming-data-in-a-flow.adoc[Transforming Data in a Flow]

Previous: xref:understanding-the-mule-message.adoc[Understanding the Mule Message]