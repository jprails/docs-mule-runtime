...
:page-aliases: mule-object-stores.adoc
...
[NOTE]
====
To use the xref:modules/object-store-module-reference.adoc[Object Store module] in Anypoint Studio, you must first download the *Object Store Connector* from the xref:2.0@exchange::exchange1.adoc[Anypoint Exchange].

image::modules/object-store-in-exchange.png[]
====
...
* When configuring an xref:filters/idempotent-filter.adoc[idempotent message filter] or until a successful scope occurs.
* When configuring a custom component that must use an object store to persist information.
* When storing or retrieving information from a Mule flow through the Object Store module, http://mulesoft.github.io/objectstore-connector/1.3.3/apidocs/mule/objectstore-config.html[available as an extension].

Mule provides two types of object stores:

* *In-memory store* – Prior to Mule 3.5.0, in-memory store was the default. As of Mule 3.5.0, persistent store is the default. For more information, see "Object Stores and Clustering" in the xref:scopes/cache-scope.adoc[Cache Scope] document.
...
[TIP]
As of Mule 3.5.0, the default object store created for all new caching strategies supports cluster mode out of the box, with the exception of the cache scope which uses the old default caching strategy. For details, see the Object Stores and Clustering section in xref:scopes/cache-scope.adoc[Cache Scope].

== Adding Object Store Components

. Install the Object Store component available in Anypoint Exchange +
+
image::modules/install-object-store-connector.png[install+object+store+connector]
...
+
image::modules/object-store-in-flow.png[]
+
. Click it to access its properties editor.
+
image::modules/object-store-properties-editor.png[]
...
. xref:filters/idempotent-filter.adoc[idempotent filter] with an in-memory object store
...
** http://mulesoft.github.io/objectstore-connector/1.3.3/apidocs/mule/objectstore-config.html[Object Store module], available as a Mule extension.
** http://mulesoft.github.io/objectstore-connector/[Object Store API].
* Learn about the xref:object-store::managing-application-data-with-object-stores.adoc[CloudHub implementation of object stores].