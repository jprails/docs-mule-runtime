...
You can also add custom xref:modules/enabling-ws-security.adoc[*Token Validators*] to the SOAP Component.
...
To the Mule flow that involves a Web service, add a xref:soap-component-reference.adoc[*SOAP Component*] to publish, consume, or proxy a Web service. Complete the following steps to configure WS-security on the SOAP component.
...
. Click the image:modules/add.png[] icon in the *Add Configuration Element* pane to create a new key-value pair.
+
...
+
[TIP]
If you have not yet created any beans, click the image:modules/add.png[] button to open a new properties panel in which you can create and configure a new bean. The bean imports the Java class you have built to specify the custom validator's override behavior.
+
...
* xref:soap-component-reference.adoc[SOAP component]