...
:page-aliases: enabling-ws-security.adoc
...
For more information, see https://www.mulesoft.com/exchange#!/soap-web-service-security[SOAP Web Service Security] in Anypoint Exchange. You can also use the xref:3.7@connectors::web-service-consumer.adoc[Web Service Consumer] to provide username token and timestamp support for WS-Security.
...
To the Mule flow that involves a Web service, add a xref:components/cxf-component-reference.adoc[CXF Component] to publish, consume, or proxy a Web service. Complete the following steps to configure WS-security on the SOAP component.
...
+
image::modules/security.png[]
+
. Click the
image:modules/add.png[(plus)] icon in the *Add Configuration Element* pane to create a new key-value pair.
. Click *cxf:property* key and Enter a key, such as `action`, in the *Key* field and press *Enter*:
+
image::modules/key-value-pair.png[key_value_pair]
+
[NOTE]
====
...
+
image::modules/open-and-specify-value.png[]
+
...
+
image::modules/token-validators.png[token_validators]
+
...
image:modules/add.png[(plus)] button to open a new properties panel in which you can create and configure a new bean. The bean imports the Java class you have built to specify the custom validator's override behavior.
====
+
image::modules/token-validators-selected.png[token_validators_selected]
+
...
http://www.mulesoft.org/schema/mule/cxf/3.7/mule-cxf.xsd[View the Mule 3.7 CXF schema].
...
* Learn more about configuring a xref:components/cxf-component-reference.adoc[CXF component] in your Mule application.
* See https://www.mulesoft.com/exchange#!/?types=example[examples].