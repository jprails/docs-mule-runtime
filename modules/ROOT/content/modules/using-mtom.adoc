...
To configure an MTOM client with the Web Service Consumer, you must specify the `mtomEnabled` attribute.  The message payload should include a sub-section that holds attachments (such as text, images, or applications) that travel with the message.  For more information on attachments, see xref:transformers/attachment-transformer-reference.adoc[Attachment Transformer Reference].
