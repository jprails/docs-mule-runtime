...
The XQuery Module gives users the ability to perform XQuery transformations on XML messages in Mule. This works in a very similar way to the xref:3.6@modules/xslt-transformer.adoc[XSLT Transformer] shipped with Mule.
...
If you convert to Mule 3.6 and later, more information on XML can be found in xref:3.8@modules/xpath.adoc[XPath3] guide.

For examples on using the XQuery transformer, including the use of features and improvements in version 3.0, see xref:3.7@modules/xquery-support.adoc[XQuery Support].
