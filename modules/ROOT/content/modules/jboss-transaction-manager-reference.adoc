...
:page-aliases: jboss-transaction-manager-reference.adoc
:keywords: mule, studio, jboss, bpms

This module enables Mule to use the JBoss transaction manager (previously Arjuna) to configure xref:transactions/xa-transactions.adoc[XA transactions]. Developers can configure one Transaction Manger per Mule instance. For more information, see http://www.jboss.org/jbosstm/[JBoss Transactions], xref:deploy/deploying-mule-to-jboss.adoc[Deploying Mule to JBoss], xref:transactions/transaction-management.adoc[Transaction Management], xref:modules/jboss-jbpm-module-reference.adoc[JBoss jBPM Module Reference].
