...
The Bookstore example is configured to run in an embedded xref:transports/jetty-transport-reference.adoc[Jetty servlet container] from within the Mule server.
...
|===
|Flow |Description
|`CatalogPublicInterface` |Listens for catalog requests using an xref:http-endpoint-reference.adoc[HTTP Inbound Endpoint], then uses the xref:modules/cxf-module-reference.adoc[CXF Module] to call the Catalog Service using the `getBooks()` method (defined in the `CatalogServiceImpl.java` class)
