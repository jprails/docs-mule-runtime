...
image:mulestudio-1.png[mulestudio(1)] This exercise is designed to introduce you to Mule Studio and help you get comfortable building applications.
...
* a xref:components/logger-component-reference.adoc[*Logger*], which logs messages or activity based on the value of a Mule expression. Find this in the Components section of the palette at the right of the screen.
* a xref:transformers/set-payload-transformer-reference.adoc[*Set Payload Transformer*], which modifies your payload into a "Hello, World" message. Find this in the Transformers section of the palette at the right of the screen.
...
+
image::studio-logger-config.png[Studio_Logger_config]
+
The string `#[message.payload]` is a simple xref:mel/mule-expression-language-mel.adoc[Mule expression] which evaluates to the current payload of the message as it passes this point in the flow. Including this message here instructs Mule to log this information in the application log files, which can be useful in more complex use cases, when you need to track the payload at different points in your flow.
...
+
This indicates that this field supports xref:mel/mule-expression-language-mel.adoc[Mule expressions], which take the form of a string enclosed with "\#[]". If you enter a Mule expression here, Mule evaluates it at runtime and returns or uses the results for further processing. Note that this field also accepts literals, so you can enter any string here to instruct Mule to set that string as your new payload. In this tutorial, however, you will use an expression to create a dynamic message.
...
To achieve this, you'll need to add another message processor to your flow to direct Mule to write your payload to a xref:file-endpoint-reference.adoc[file]. You'll also need to use another Mule expression that you haven't seen before in order to log and use the unique message id. Use the hints below if you need help. Go!

=== image:icon-question-blue-big.png[icon-question-blue-big(1)] Hints
...
* Want to learn more about Mule Expression Language (MEL)? Check out the xref:mel/mule-expression-language-mel.adoc[complete reference] documentation.
* Want to try a Hello World example using xref:cloudhub::index.adoc[CloudHub] instead? Check out xref:general:getting-started:deploy-to-cloudhub.adoc[Deploy to CloudHub].
