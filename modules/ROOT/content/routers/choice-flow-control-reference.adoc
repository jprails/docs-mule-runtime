...
In Studio, drag the *Choice* icon from the Studio palette to the canvas, positioning it within the sequence of xref:3.7@flows/elements-in-a-mule-flow.adoc[building blocks] that form the flow (below).

image::routers/choice-flow-2.png[choice+flow+2]
...
+
image::routers/drag-to-choice.png[drag_to_choice]
...
+
image::routers/drag-session.png[drag_session]
...
+
image::routers/drag-attachment.png[drag_attachment]
...
+
image::routers/drag-ftp.png[drag_FTP]

. Open the Choice Flow Control's Properties Editor (below)
+
image::routers/choice1.png[]
...
+
image::routers/choice-select-default-1.png[choice+select+default+1]

. Check the *Otherwise* box (see below), then click *OK*.  +
+
image::routers/select-default.png[select+default]
...
+
image::routers/select-default-2.png[select+default+2]
...
* For more information on the Choice Flow Control, see the xref:3.6@routers/routers.adoc[Choice] section on the Routing Message Processors page.