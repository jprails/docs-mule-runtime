...
In Studio, drag the *Choice* icon from the Studio palette to the canvas, positioning it within the sequence of xref:flows/elements-in-a-mule-flow.adoc[building blocks] that form the flow (below).
...
+
image::routers/choiceclickhere.png[ChoiceClickHere]
+
. In the Route Properties dialog, enter: `#[flowVars.language == 'Spanish']`
+
image::routers/choicerouteproperties.png[ChoiceRouteProperties]
+
...
+
image::routers/holaamigos.png[HolaAmigos]
+
...
+
image::routers/varsetlangtoenv.png[VarSetLangToEnv]
+
...
+
image::routers/setpayloadenglish.png[SetPayLoadEnglish]
+
...
image::routers/finalflow.png[FinalFlow]

== See Also

* For more information on the Choice Flow Control, see the xref:routers/routers.adoc[Choice] section on the Routing Message Processors page.