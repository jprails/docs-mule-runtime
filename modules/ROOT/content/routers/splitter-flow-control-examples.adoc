...
:page-aliases: splitter-flow-control-examples.adoc

== Example Splitting and Aggregating with Asynchronous Flows

This example builds upon the xref:routers/splitter-flow-control-reference.adoc#basic-splitter-example[Splitter basic example]. Follow the steps below to run message fragments in asynchronous flows and then aggregate them back into a single message.
