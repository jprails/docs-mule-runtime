...
[NOTE]
Weigh the pros and cons of using a splitter and aggregator in your flow, versus using a "Foreach" scope xref:scopes/foreach.adoc#using-foreach-versus-splitter-and-aggregator-pairs[here].
...
+
image::routers/splitter-example.png[splitter_example]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
+
image::routers/splitter-flow.png[splitter+flow]
+

 . Run the Mule project.
. Send the HTTP Listener an HTTP request that includes a body with an attached XML file. You can use a browser extension such as Postman (Google Chrome), or the http://curl.haxx.se/[curl] command line utility.
...
+
image::routers/variables-diagramv2.png[variables+diagramv2]
+
...
image::routers/diagram-ag-2.png[diagram+ag+2]
...
image::routers/collection-agg.png[collection_agg]
...
+
image::routers/splitter-flow-2.png[splitter+flow+2]
+
...
+
image::routers/splitter-flow-3.png[splitter+flow+3]
+

. Configure the two VM connectors. Change both their *Queue Path* to `step2`.
+
image::routers/vm4.png[]
+
...
+
image::routers/splitter-flow-4.png[splitter+flow+4]
+
. Run the Mule project.
...
+
image::routers/splitter-flow-5.png[splitter+flow+5]
+
...
+
image::routers/splitter-flow-6.png[splitter+flow+6]
+
...
* Learn more about xref:mel/mule-expression-language-mel.adoc[Mule Expression Language (MEL)].
* Learn about other xref:routers/all-flow-control-reference.adoc[Flow Control] elements.
* Read more about the xref:scopes/async-scope-reference.adoc[Async Scope].