...
The timeout setting determines how long Mule should wait for replies before returning the result. The default value is determined by the value of the `defaultSynchronousEventTimeout` attribute that has been configured for the Mule instance. (For more information, see xref:mule-configuration/global-settings-configuration-reference.adoc[Global Settings Configuration Reference].) You can also specify an independent timeout value for asynchronous replies for a given service using the optional `timeout` attribute on the `async-reply` element.
