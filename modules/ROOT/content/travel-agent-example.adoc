...
The *Travel Agent Web Service* example application consists of several xref:flows/index.adoc[flows] which collaborate to process end user requests. Together, the flows in this application expose a *SOAP Web service* which accepts end user requests to reserve flights and book hotel rooms.
...
The Mule ESB Travel Agent application contains multiple xref:flows/index.adoc[flows and subflows] which cooperate to expose a *SOAP Web service*. The sections below offer flow-by-flow details of the Travel Agent Web service’s actions as it processes end user requests.
...
The TravelAgentRequest flow must, eventually, pass the message to the <<ConfirmReservations Flow>> flow via the request-response **xref:http-endpoint-reference.adoc[HTTP Endpoint].** Because the Reservations flow only accepts XML messages, the TravelAgentRequest flow must first transform the data format of the message payload from JSON to XML. To perform the transformation, this flow uses an  **xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper]**.

This flow next employs an **xref:transformers/xslt-transformer-reference.adoc[XSLT Transformer]** to wrap the message in a SOAP envelope. Then, it uses the HTTP endpoint to push the message to the Reservations flow for processing.
...
This flow invokes several **xref:flows/index.adoc[subflows]** in the Travel Agent application to book reservations and charge a commission.
...
Next, the Reservations flow passes the message through a **xref:filters/message-filter.adoc[Message Filter]** to confirm that the preceding subflow identified a room. The message filter examines the message payload to determine whether Mule should process the message further.
...
Next, the **xref:transformers/session-variable-transformer-reference.adoc[Session Variable Transformer]** adds the flight and hotel costs, then appends the total travel cost to the message as a session variable.
...
Mule then subjects the message to a **xref:transformers/variable-transformer-reference.adoc[Variable Transformer]** which collaborates with a **xref:scopes/foreach.adoc[Foreach]** iterative processor to identify the least expensive room on the list.
...
This flow receives a message through a request-response *VM Endpoint*, then uses a flow reference component to invoke a subflow to authorize credit card payments. Once authorized to make payments, Mule passes the message to a set variable transformer. This transformer, `Set Rollback = CREDIT_CARD`, and the `Set Rollback = FLIGHT` transformer (further in the flow) collaborate with a **xref:exceptions/choice-exception-strategy.adoc[Choice Exception Strategy]** to handle errors.
...
* If the variable contains a `CREDIT_CARD` value, the choice exception strategy directs the message through the first ** xref:exceptions/catch-exception-strategy.adoc[Catch Exception Strategy] **which: +
...
In flows that conduct http://en.wikipedia.org/wiki/Transaction_processing[transactions], you would use a *xref:exceptions/rollback-exception-strategy.adoc[Rollback Exception Strategy]* to handle errors (i.e. http://en.wikipedia.org/wiki/Rollback_(data_management)[rollback transactions]).
...
Both catch exception strategies in the ConfirmReservations flow invoke this *ThrowException* flow. Containing only a **xref:components/groovy-component-reference.adoc[Groovy Script Component]**, this flow follows a script to throw a `RuntimeException` and return a message that reads `Unable to confirm reservations`.
...
The `/commission` Web service passes requests through a request-response HTTP endpoint to a **xref:routers/choice-flow-control-reference.adoc[Choice Flow Control]**. The flow control routes requests to one of two *Set Payload Transformers* to set a commission rate on the message.
...
Rather than use an expression transformer like the ConfirmFlight subflow, this subflow uses an xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper] transformer to prepare a request for the hotels’ SOAP Web service.
...
* For more information on configuring the Anypoint DataMapper, see xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper Transformer Reference].
...
* For more information on the specific exception strategies in this example, see xref:exceptions/choice-exception-strategy.adoc[Choice Exception Strategy] and xref:exceptions/catch-exception-strategy.adoc[Catch Exception Strategy].
* For more information on routing messages, see xref:routers/choice-flow-control-reference.adoc[Choice Flow Control Reference].
* For more information on enriching messages, see xref:studio-scopes.adoc[Studio Scopes].
* For more information on setting variables on messages, see xref:transformers/variable-transformer-reference.adoc[Variable Transformer Reference] and xref:transformers/session-variable-transformer-reference.adoc[Session Variable Transformer Reference].
* For more information on caching Web service responses, see xref:scopes/cache-scope.adoc[Cache Scope].
...
* For more information on filtering messages, see xref:filters/message-filter.adoc[Message Filter].