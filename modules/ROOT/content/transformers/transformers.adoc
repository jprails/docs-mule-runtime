...
NOTE: The DataMapper transformer has been deprecated. Use xref:dataweave/index.adoc[DataWeave] instead.
...
|image:transformers/groovy-transformer-icon.png[Groovy icon] |Groovy |Implements a Groovy script transformer backed by a Groovy script engine. All of the configuration fields for this transformer are covered in the <<Common Transformer Configuration Fields>> section of this page.
|image:transformers/javascript-transformer-icon.png[JavaScript icon] |JavaScript |Implements a JavaScript transformer backed by a JavaScript script engine. See xref:transformers/script-transformer-reference.adoc[Script Transformer Reference].
|image:transformers/python-transformer-icon.png[Python icon] |Python |Implements a script transformer backed by a Python script engine. See xref:transformers/script-transformer-reference.adoc[Script Transformer Reference].
|image:transformers/ruby-transformer-icon.png[Ruby icon] |Ruby |Implements a script transformer backed by a Ruby script engine. See  xref:transformers/script-transformer-reference.adoc[Script Transformer Reference]
|image:transformers/script-transformer-icon.png[Script icon] |Script |Implements a script transformer backed by a JSR-223- compliant script engine, such as a Groovy, Javascript, Python, or Ruby. See xref:transformers/script-transformer-reference.adoc[Script Transformer Reference].
|===
...
|===
|Icon |Transformer |Description
|image:transformers/byte-array-to-object.png[] |Byte Array to Object |Converts a byte array to an object, either by de-serializing the array or converting it to a string).

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/byte-array-to-serializable.png[] |Byte Array to Serializable |Deserializes a byte array, thus converting it into an object.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/byte-array-to-string.png[] |Byte Array to String |Converts a byte array to a string.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/byte-array-to-mime.png[] |Byte Array to MIME |Changes the MIME type for a byte array.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/file-to-byte-array.png[] |File to Byte Array |Reads the contents of a java.io.File into a Byte array.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/file-to-string.png[] |File to String |Reads the contents of a `java.io.File` into a `java.lang.String` object.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/java-transformer-icon.png[] |Java |Transforms the data from one format to another.

*Documentation*: xref:transformers/java-transformer-reference.adoc[Java Transformer Reference]
|image:transformers/jms-message-to-object.png[] |JmsMessage to Object (Enterprise Edition) |Converts a JMS message into an object by extracting the message payload.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/json-to-object.png[] |JSON to Object |Converts a Json-encoded object graph into a Java Object.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/object-to-byte-array.png[] | Object to Byte Array |Converts an object to a byte array.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/object-to-string.png[] |Object to String |Converts program code types into readable text strings Used for debugging.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/object-to-xml-alt.png[object-to-xml] |Object to XML |Converts a Java Object into XML code using XStream.

*Documentation*: xref:transformers/object-to-xml-transformer-reference.adoc[Object-to-XML Transformer Reference]
|image:transformers/serializable-to-byte-array.png[Serializable to Byte Array] |Serializable to Byte Array |Converts a Java Object to a byte array by serializing the object.

*Documentation*: xref:transformers/object-to-xml-transformer-reference.adoc[Object-to-XML Transformer Reference]
|image:transformers/string-to-byte-array.png[] |String to Byte Array |Converts a string into a byte array.

*Documentation*: <<Common Transformer Configuration Fields>>
|image:transformers/xml-to-object.png[] |XML to Object |Uses XStream to convert XML into Java Bean graphs.

*Documentation*: xref:modules/domtoxml-transformer.adoc[DOM/XML Transformers]
|image:transformers/xml-to-dom.png[] |XML to DOM |Converts raw bytes into an in memory representation of a DOM document.

*Documentation*: xref:modules/domtoxml-transformer.adoc[DOM/XML Transformers]
|image:transformers/dom-to-xml.png[] |DOM to XML |Converts any type of parsed XML into raw bytes.
...
|===
|Icon  |Transformer |Description
|image:transformers/append-string-icon.png[] |Append string |Appends a string to a message payload.

*Documentation*: xref:transformers/append-string-transformer-reference.adoc[Append String Transformer Reference]
|image:transformers/expression-transformer-icon.png[] |Expression |Evaluates one or more expressions within the message, then transforms the message according to the results of its evaluation.

*Documentation*: xref:transformers/expression-transformer-reference.adoc[Expression Transformer Reference]
|image:transformers/transformer-reference-icon.png[] |Transformer Reference |References a transformer that is defined as a global element.

*Documentation*: xref:transformers/xslt-transformer-reference.adoc[Transformer Reference]
|image:transformers/xslt-transformer-icon.png[] |XSLT |Transforms XML using XSLT.

*Documentation*: xref:transformers/xslt-transformer-reference.adoc[XSLT Transformer Reference]
|===
...
|===
|Icon  |Transformer |Description
|image:transformers/sap-object-to-xml.png[] |SAP-Object-to-XML (Enterprise Edition) |Transforms a SAP object representing a JCo function or IDoc document into its XML representation.

*Documentation*: https://mulesoft.github.io/sap-transport/[SAP Connector].
|image:transformers/xml-to-sap-function-bapi.png[] |XML to Function (BAPI) (Enterprise Edition) |Reads the XML representing a JCo function from `java.io.InputStream`, `java.lang.String` or `byte[]` to build the SAP object expected by the SAP transport.

*Documentation*: https://mulesoft.github.io/sap-transport/[SAP Connector].
|image:transformers/xml-to-sap-idoc.png[] |XML to IDOC (Enterprise Edition) |Reads the XML representing an IDOC document from `java.io.InputStream`, `java.lang.String`, or `byte[]` to build the SAP object expected by the SAP transport.
...
|===
|Icon  |Transformer |Description
|image:transformers/attachment-transformer-icon.png[] |Attachment |In contrast to the *Message Enricher Scope* or the *Append String Transformer*, the *Attachment Transformer* does not add to the string that typically composes the main data payload. Instead, this transformer specifies an attachment to append to each message being processed through the flow. If the name or the value of the attachment is defined through an expression, the exact identity (and content) of the attachment can be calculated at run-time, with the possibility that each message receives a different payload. Typically, this attachment is treated as a separate, secondary part of the outbound payload.

*Documentation*: xref:transformers/attachment-transformer-reference.adoc[Attachment Transformer Reference]
|image:transformers/property-icon.png[] |Property |This transformer allows you to specify a property, which is typically applied to the message header. The "life span" of such a property extends from the moment it is created until the message is passed to an outbound endpoint.

*Documentation*: xref:transformers/property-transformer-reference.adoc[Property Transformer Reference]
|image:transformers/variable-icon.png[] |Variable |This transformer facilitates dynamic, run-time determination of the specified variable's value based on the content of the current message or the current state of the Mule environment. Mule can then use this value to alter the payload content or the processing steps ultimately assigned to the current message. This type of variable remains active as long as the message remains within the flow in which the variable was invoked. As soon as the message gets passed to a different flow via a transport, the variable becomes inactive.

*Documentation*: xref:transformers/variable-transformer-reference.adoc[Variable Transformer Reference]
|image:transformers/session-variable-icon.png[] |Session Variable |This transformer resembles the Variable transformer, except the Session Variable set by this transformer persists as long as the associated message remains within the Mule application, even though the message may be processed through multiple flows.

*Documentation*: xref:transformers/session-variable-transformer-reference.adoc[Session Variable Transformer Reference]
|===
...
The transformers below are all related to the deprecated xref:3.8@connectors::http-connector-deprecated.adoc[endpoint-based HTTP Connector]. They have all been made unnecessary, as the new xref:3.8@connectors::http-connector.adoc[HTTP Connector]'s features cover the use cases these were built to serve.
