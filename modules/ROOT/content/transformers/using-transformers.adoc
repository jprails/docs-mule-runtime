...
:page-aliases: using-transformers.adoc
...
[TIP]
The most powerful tool for transforming a message is the xref:3.8@dataweave/index.adoc[DataWeave Transformer], which cannot only change the format of the message, but also rename, aggregate, split, or rearrange the fields in it in any way you desire.
...
* http://msdn2.microsoft.com/en-us/library/aa480061.aspx[Messaging Patterns in Service Oriented Architecture, Part 2]
* http://it.toolbox.com/blogs/composite-apps/transformation-in-a-soa-12186[Transformation in a SOA]

== Available Transformers

Following are the transformers available with Mule. Some transformers are specific to a transport or module. For more information, see xref:transports/transports-reference.adoc[Transports Reference] and xref:modules/modules-reference.adoc[Modules Reference]. For a complete reference to the elements and attributes for the standard Mule transformers, see xref:transformers/transformers-configuration-reference.adoc[Transformers Configuration Reference].

=== Basic

The basic transformers are in the https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/transformer/simple/package-summary.html[org.mule.transformer.simple] package. They do not require any special configuration. For details on these transformers, see xref:transformers/transformers-configuration-reference.adoc[Transformers Configuration Reference].
...
|ExpressionTransformer |Evaluates one or more expressions on the current message and return the results as an Array. For details, see xref:mel/mule-expression-language-mel.adoc[Mule Expression Language MEL].
...
https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/module/xml/transformer/package-summary.html[org.mule.module.xml.transformer]
package. They provide the ability to transform between different XML formats, use XSLT, and convert to POJOs from XML. For information, see xref:modules/xml-module-reference.adoc[XML Module Reference].

[%header,cols="2*"]
|===
|Transformer |Description
|xref:modules/xmlobject-transformers.adoc[XmlToObject < - > ObjectToXml] |Converts XML to a Java object and back again using http://x-stream.github.io/[XStream].
|xref:modules/jaxb-transformers.adoc[JAXB XmlToObject < - > JAXB ObjectToXml] |Converts XML to a Java object and back again using the http://java.sun.com/developer/technicalArticles/WebServices/jaxb/[JAXB] binding framework.
|xref:modules/xslt-transformer.adoc[XSLT] |Transforms XML payloads using XSLT.
|xref:modules/xquery-transformer.adoc[XQuery] |Transforms XML payloads using http://en.wikipedia.org/wiki/XQuery[XQuery].
|xref:modules/domtoxml-transformer.adoc[DomToXml < - > XmlToDom] |Converts DOM objects to XML and back again.
|xref:modules/xmltoxmlstreamreader-transformer.adoc[XmlToXMLStreamReader] |Converts XML from a message payload to a StAX XMLStreamReader.
|xref:modules/xpath-extractor-transformer.adoc[XPath Extractor] |Queries and extracts object graphs using XPath expressions using JAXP.
|xref:modules/jxpath-extractor-transformer.adoc[JXPath Extractor] |Queries and extracts object graphs using XPath expressions using JXPath.
|xref:transformers/xmlprettyprinter-transformer.adoc[XmlPrettyPrinter] |Allows you to output the XML with controlled formatting, including trimming white space and specifying the indent.
|===

=== JSON

The JSON transformers are in the https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/module/json/transformers/package-summary.html[org.mule.module.json.transformers] package. They provide the ability to work with JSON documents and bind them automatically to Java objects. For information, see xref:transformers/native-support-for-json.adoc[Native Support for JSON].

=== Scripting

The xref:modules/scripting-module-reference.adoc[Scripting] transformer transforms objects using scripting, such as JavaScript or http://www.groovy-lang.org/[Groovy] scripts. This transformer is in the
https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/module/scripting/transformer/package-summary.html[org.mule.module.scripting.transformer] package.

=== Encryption

The encryption transformers are in the https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/transformer/encryption/package-summary.html[org.mule.transformer.encryption] package.

[%header,cols="2*"]
|===
|Transformer |Description
|xref:transformers/transformers-configuration-reference.adoc[Encryption < - > Decryption] |A pair of transformers that use a configured EncryptionStrategy implementation to encrypt and decrypt data.
|===

=== Compression

The compression transformers are in the https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/transformer/compression/package-summary.html[org.mule.transformer.compression] package. They do not require any special configuration.

[%header,cols="2*"]
|===
|Transformer |Description
|xref:transformers/transformers-configuration-reference.adoc[GZipCompressTransformer < - > GZipUncompressTransformer] |A pair of transformers that compress and uncompress data.
|===

=== Encoding

The encoding transformers are in the
https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/transformer/codec/package-summary.html[org.mule.transformer.codec]
package. They do not require any special configuration.

[%header,cols="2*"]
|===
|Transformer |Description
|xref:transformers/transformers-configuration-reference.adoc[Base64Encoder <-> Base64Decoder] |A pair of transformers that convert to and from Base 64 encoding.
|xref:transformers/transformers-configuration-reference.adoc[XMLEntityEncoder <-> XMLEntityDecoder] |A pair of transformers that convert to and from XML entity encoding.
|===

=== Email

The Email transport provides several transformers for converting from email to string, object to MIME, and more. For details, see xref:transports/email-transport-reference.adoc[Email Transport Reference].

=== File

The File transport provides transformers for converting from a file to a byte array (byte[]) or a string. For details, see xref:transports/file-transport-reference.adoc[File Transport Reference].

=== HTTP

The HTTP connector provides several transformers for converting an HTTP response to a Mule message, map or string, and for converting a message to an HTTP request or response. For details, see xref:3.6@connectors::http-connector.adoc[HTTP Connector].

=== JDBC +
_*Enterprise*_

The Mule Enterprise version of the JDBC transport provides transformers for moving CSV and XML data from files to databases and back. For details, see xref:transports/jdbc-transport-reference.adoc[JDBC Transport Reference].

=== JMS

The xref:transports/jms-transport-reference.adoc[JMS Transport Reference] and xref:transports/mule-wmq-transport-reference.adoc[Mule WMQ Transport Reference] (enterprise only) both provide transformers for converting between JMS messages and several different data types.

=== Strings and Byte Arrays

The xref:transports/multicast-transport-reference.adoc[Multicast Transport Reference] and xref:transports/tcp-transport-reference.adoc[TCP Transport Reference] both provide transformers that convert between byte arrays and strings.

=== XMPP

The XMPP transport provides transformers for converting between XMPP packets and strings. For details, see xref:transports/xmpp-transport-reference.adoc[XMPP Transport Reference].

=== Custom

Mule supports the ability to build xref:transformers/custom/creating-custom-transformers.adoc[Custom Transformer]. Build custom transformers to meet specific data conversion needs in your application.
...
If set to true, the transformer  ignores any data that it does not know how to transform, but any transformers following it in the current chain will be called. If set to false, the transformer will also ignore any data that it does not know how to transform, but no further transformations will take place.
