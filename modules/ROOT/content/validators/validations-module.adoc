...
:page-aliases: validations-module.adoc
:keywords: validate, validations, filter, if, assert, exception, confirm

The Validations module provides an easy out-of-the-box way to verify that the content of a message in your flow matches a given set of criteria. The main advantage this has over using xref:filters/filters.adoc[Filters] is traceability, as filters all raise identical exceptions, making it hard for you to know where the exception was caused. Validators, on the other hand, raise an exception with a meaningful message attached. You can optionally customize this message and even the type of exception you want it to throw.

*Note:* This feature is supported by the Mule runtime as of xref:release-notes::mule-runtime/3.7.0-release-notes.adoc[Mule ESB 3.7.0] in both Community and Enterprise editions. However, Studio support only became available starting with the September 2015 release.
...
* In case you want to throw an Exception type that lacks this constructor, or in which its creation is not trivial, or in which you want it to contain additional information, you can xref:validators/building-a-custom-validator.adoc[build a custom validator].
...
image::validators/validator-is-email.png[validator]
...
Also, in most cases you can invoke a validator via a xref:mel/mule-expression-language-mel.adoc[Mule Expression Language (MEL)] expression. The purpose of exposing validators through MEL is to use them in decision making. Unlike the message processor version, these work as boolean functions and don't produce an output message, they just return true when the validation is successful or false otherwise. All of the validators are available through a *validator* variable in the MEL context.
...
There are several different types of validator that out of the box cover most of the common use cases you might run into. If none of the validators described below cover what you need, you can also xref:validators/building-a-custom-validator.adoc[build a custom validator].
...
image::validators/validator-is-email.png[email]
...
image::validators/validator-regex.png[validator]
...
image::validators/validator-is-time.png[validator]
...
image::validators/validator-is-not-empty.png[validator]
...
image::validators/validator-is-empty.png[validator]
...
image::validators/validator-size.png[validator]
...
image::validators/validator-is-not-null.png[validator]
...
image::validators/validator-is-null.png[validator]
...
image::validators/validator-is-number.png[validator]
...
image::validators/validator-is-ip.png[validator]
...
image::validators/validator-is-url.png[validator]
...
image::validators/validator-is-true.png[validator]


image::validators/validator-is-false.png[validator]
...
+
image::validators/validator-global-element.png[validator]

Then select the validation configuration:
+
image::validators/validator-global-element2.png[validator]
...
image::validators/validator-internationalization.png[validator]
...
image::validators/validator-all.png[validator]
...
The example above includes an `all` validator that simultaneously validates the following things:
...
This page assumes that you have `<mule.version></mule.version>` property defined in your `pom.xml`. If not, you can replace `${mule.version}` with the Mule runtime version that is being used.


== See Also

* http://blogs.mulesoft.com/dev/tech-ramblings/introducing-the-validations-module/[Blogpost]
