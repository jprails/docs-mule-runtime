...
As the objective of this quick tutorial is get something built and running fast, this document doesn't spend much time explaining components. More detailed explanations and concepts exist in the xref:general:getting-started:index.adoc[Anypoint Fundamentals], but for now, let's just see Mule work: you're about to build a very simple application in Studio, deploy it to Studio's embedded server, then interact with the application through a Web browser.

== Assumptions

This tutorial assumes that you have xref:6@studio::download-and-launch-anypoint-studio.adoc[downloaded, installed and launched Mule Studio],  and that you've read through the xref:5@studio::index.adoc[Mule Studio Essentials].
...
Mule Studio comes bundled with its own embedded server, ideal for testing deployments as you code. You can deploy your application on the embedded server to see it working and perform any debugging activities before deploying to a production environment, such as an xref:runtime-manager::deploying-to-your-own-servers.adoc[on-premises server] or xref:cloudhub::index.adoc[CloudHub].
...
* *NEXT STEP*: Dive into the more extensive *First Day* content starting with xref:concepts/index.adoc[Mule Concepts].
* Happier using the command line to run applications? Learn more about xref:deploy/starting-and-stopping-mule-esb.adoc[starting and stopping Mule].
