...
* When the Stock Quote Service replies, the application first transforms the result using the xref:transformers/xslt-transformer-reference.adoc[XSLT Transformer], then uses the xref:transformers/xml-to-object-transformer-reference.adoc[XML to Object Transformer] to transform it to a StockQuote Java bean.
