...
This example adds a new flow to the existing 'core-concepts' project created in the xref:configuring-an-endpoint.adoc[Configuring an Endpoint] example.
...
. We're ready to create our new flow. Right click on the project tree and select New --> Mule Message Flow. When the New Mule Flow dialog appears, enter _hello_ as the name and click Finish.


. Now drag a HTTP endpoint and a component from the palette onto the canvas. Double-click the component to open its properties dialog and set the Display Name to _Hello_.

. Double click on the HTTP endpoint to edit its properties. set the host to _localhost_ and the port to _8082_. The port for this connector is 8082 not 8081 as in the previous examples. This is because we added this flow to our existing project and when it starts both the _echo_ and _hello_ flows will be loaded; a port conflict would happen if both flows listened on the same port.
...
. In a browser window goto `+http://localhost:8082/Fred+`.
...
* xref:components/configuring-components.adoc[Mule Components]
* xref:components/configuring-java-components.adoc[Configuring Java Components]
* xref:components/developing-components.adoc[EntryPoint Resolvers]
