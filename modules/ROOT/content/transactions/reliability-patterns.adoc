...
:page-aliases: 3.5@mule-management-console::reliability-patterns.adoc, reliability-patterns.adoc
...
image::transactions/reliability-pattern.png[Reliability_Pattern]
...
❶ The flow specifies a synchronous processing strategy (`processingStrategy="synchronous"`). In a synchronous processing strategy, the entire flow is processed in the receiver thread. This ensures that the transfer to the VM endpoint happens in the same thread. See xref:3.7@flows/flow-processing-strategies.adoc[Flow Processing Strategies] for further details about the synchronous processing strategy. +
