...
:page-aliases: 3.3@mule-management-console::reliability-patterns.adoc, reliability-patterns.adoc
...
image::transactions/reliability-pattern.png[Reliability_Pattern+(1)]
...
image::transactions/reliabilitypatternwithjms.png[ReliabilityPatternwithJMS]
...
❶ The flow specifies a synchronous processing strategy (`processingStrategy="synchronous"`). In a synchronous processing strategy, the entire flow is processed in the receiver thread. This ensures that the transfer to the VM endpoint happens in the same thread. See xref:3.4@flows/flow-processing-strategies.adoc[Flow Processing Strategies] for further details about the synchronous processing strategy. +
