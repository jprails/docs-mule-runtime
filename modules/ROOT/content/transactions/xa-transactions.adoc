...
:page-aliases: xa-transactions.adoc
...
[TIP]
Our default and recommended global XA Transaction Manager is *Bitronix*, read xref:transactions/using-bitronix-to-manage-transactions.adoc[Using Bitronix to Manage Transactions] for details on how to implement it in Mule. The Bitronix transaction manager can be embedded into your Mule application, in which case you will not have to configure a connection to an external global XA transaction manager.
...
This property is only applicable if you are using the the default transport, `XaTransactedJmsMessageReceiver` , on JMS inbound connectors configured to use XA transactions. If you are using JBoss transactions, please read xref:modules/jboss-transaction-manager-reference.adoc[JBoss Transaction Manager Reference] to learn how to configure the `timeout` value.
...
You can embed a global transaction manager inside your Mule application. Refer to xref:transactions/using-bitronix-to-manage-transactions.adoc[Using Bitronix to Manage Transactions] to see how to implement our default and recommended XA Transaction Manager.
...
|JBoss |  |image:transactions/check.png[(tick)] |java:/TransactionManager a|

----
org.mule.transaction.lookup.JBossTransactionManagerLookupFactory
----

|JRun |  |image:transactions/check.png[(tick)] |java:/TransactionManager a|

----
org.mule.transaction.lookup.JRunTransactionManagerLookupFactory
----

|Other |  |image:transactions/check.png[(tick)] |Specified via a `jndiName` property a|

----
org.mule.transaction.lookup.GenericTransactionManagerLookupFactory
----

|Resin |  |image:transactions/check.png[(tick)] |java:comp/TransactionManager a|

----
org.mule.transaction.lookup.Resin3TransactionManagerLookupFactory
----

|WebLogic |image:transactions/check.png[(tick)] |image:transactions/check.png[(tick)] |javax.transaction.TransactionManager a|

----
org.mule.transaction.lookup.WeblogicTransactionManagerLookupFactory
----

|WebSphere |  |image:transactions/check.png[(tick)] |proprietary API call a|
...
* Refer to xref:transactions/transaction-management.adoc[Transaction Management] for details on how to configure an XA transaction.
* Refer to xref:transactions/using-bitronix-to-manage-transactions.adoc[Using Bitronix to Manage Transactions] to see how to implement our default and recommended XA Transaction Manager +