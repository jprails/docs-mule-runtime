...
|JRun |  |image:transactions/check.png[(tick)] |java:/TransactionManager a|`org.mule.transaction.lookup.JRunTransactionManagerLookupFactory`
|Other |  |image:transactions/check.png[(tick)] |Specified via a `jndiName` property a|`org.mule.transaction.lookup.GenericTransactionManagerLookupFactory`
|Resin |  |image:transactions/check.png[(tick)] |java:comp/TransactionManager a|`org.mule.transaction.lookup.Resin3TransactionManagerLookupFactory`
|WebLogic |image:transactions/check.png[(tick)] |image:transactions/check.png[(tick)] |javax.transaction.TransactionManager a|`org.mule.transaction.lookup.WeblogicTransactionManagerLookupFactory`
|WebSphere |  |image:transactions/check.png[(tick)] |proprietary API call a| `org.mule.transaction.lookup.WebsphereTransactionManagerLookupFactory`
|===
