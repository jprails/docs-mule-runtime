...
|xref:transactions/single-resource-transactions.adoc[Single Resource Transactions] a|
...
|xref:transactions/multiple-resource-transactions.adoc[Multiple Resource Transactions] a|
...
|xref:transactions/xa-transactions.adoc[XA Transactions] a|
* Receives or sends messages to more than one resource.
...
While xref:transactions/xa-transactions.adoc[XA Transactions] offer similar functionality, xref:transactions/multiple-resource-transactions.adoc[Multiple Resource transactions] use less overhead.  XA transactions are more reliable. Both can include JDBC resources. For a discussion of different approaches – including the 1.5 phase commit concept which Multiple Resource transactions use – see the http://www.javaworld.com/javaworld/jw-01-2009/jw-01-spring-transactions.html[JavaWorld article on distributed transactions].
====

== Assumption

This document assumes that you are familiar with Mule and/or the xref:6@studio::index.adoc[Anypoint Studio interface]. Further, this document assumes you have some knowledge of transaction processing and are familiar with xref:3.6@connectors::index.adoc[Anypoint Connectors].
...
Use xref:transactions/transactions-configuration-reference.adoc[Transactions Configuration Reference] for quick access to attribute configurations.
...
_Apply a transaction to a flow that involves a single resource. See xref:transactions/single-resource-transactions.adoc[Single Resource Transaction] for details.
|XA Transaction |Apply a transaction to a flow that involves multiple resources: JMS, VM or JDBC. See xref:transactions/xa-transactions.adoc[XA Transaction] for details.
|Multi Transaction |Apply a transaction to a flow that involves multiple resources: JMS or VM. See xref:transactions/multiple-resource-transactions.adoc[Multiple Resource Transaction] for details.
...
* If you apply an XA transaction to a JMS inbound connector in your flow, you have the option of specifying the polling frequency of the queue. Access xref:transactions/xa-transactions.adoc[XA Transactions] for configuration details.
...
* Read more about xref:transactions/single-resource-transactions.adoc[Single Resource Transactions], xref:transactions/multiple-resource-transactions.adoc[Multiple Resource Transactions], and xref:transactions/xa-transactions.adoc[XA Transactions].
...
* Use the xref:transactions/transactions-configuration-reference.adoc[Transactions Configuration Reference] for quick reference to transactional attributes.