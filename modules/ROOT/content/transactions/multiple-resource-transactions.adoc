...
:page-aliases: multiple-resource-transactions.adoc
...
In the example above, Mule starts the local JMS transaction when it receives the message on the inbound connector. Mule starts the local WMQ transaction when it sends the message out on the outbound connector. In this case, because the connectors are configured as multiple resource transactions, Mule commits the WMQ transaction first, then commits the JMS transaction. Ultimately, Mule either commits both transactions successfully or fails and rolls back both of them as one virtual unit.
...
Refer to xref:3.6@transactions/transaction-management.adoc[Transaction Management] for details on how to configure a Multiple Resource transaction.