...
[NOTE]
====
This page discusses the configuration of the flow-level inbound and outbound endpoints, which are the flow-level elements of endpoint-based Anypoint Connectors. Note that none of the information on this page applies for operation-based connectors. Please see xref:3.6@connectors::index.adoc[Anypoint Connectors] for more information about connectors in general.
====
...
The following reference tables list the attributes that can be configured for the xref:3.6@connectors::generic-connector.adoc[generic endpoint] in Mule.
