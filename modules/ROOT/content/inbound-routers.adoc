...
* Transactions are supported, so the forwarding of messages can either start or join a transaction provided that the endpoint xref:transactions/transaction-management.adoc[supports transactions].
