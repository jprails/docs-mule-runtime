...
This document describes the details of the example within the context of *Mule Studio*, Mule ESB’s graphical user interface (GUI). Where appropriate, the XML configuration accompanies the Studio interface screenshots. This document assumes that you are familiar with Mule ESB and the xref:5@studio::index.adoc[Mule Studio interface]. To increase your familiarity with Mule Studio, consider completing one or more xref:5@studio::basic-studio-tutorial.adoc[Mule Studio Tutorials].
...
The **HTTP Request-Response with Logger** example application consists of one, simple xref:flows/index.adoc[flow] which receives end user HTTP requests, logs messages’ payloads, and returns the payloads to end users as HTTP responses.
...
Next, the flow uses a **xref:components/logger-component-reference.adoc[Logger Component]** to log the message payload in the application’s log files. The logger component uses a** xref:mel/mule-expression-language-mel.adoc[MEL Expression]** to determine what information in, or about, the message it should log. In this case, because it needs to log the message payload, the instructions to log read `About to echo #[message:payload]`. +
 +
...
Finally, Mule moves the message to an **xref:components/echo-component-reference.adoc[Echo Component] **which simply returns the message payload as the response to an end user. In other words, the response echoes the request.
...
* Learn more about thexref:components/logger-component-reference.adoc[Logger component].
* Learn more about the xref:http-endpoint-reference.adoc[HTTP endpoint].
* Learn more about the xref:components/echo-component-reference.adoc[Echo component].