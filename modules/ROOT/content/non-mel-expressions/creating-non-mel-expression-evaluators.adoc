...
In addition to the xref:non-mel-expressions/non-mel-expressions-configuration-reference.adoc[standard expression evaluators] provided with Mule, you can create your own evaluators. This page describes how to create a custom evaluator, as well as how to add expression support to a custom module.

== Creating the Custom Evaluator

To create a custom evaluator, the first step is to implement the https://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/api/expression/ExpressionEvaluator.html[ExpressionEvaluator] interface. This is a simple strategy interface:
