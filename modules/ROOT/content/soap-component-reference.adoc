...
Using Mule's SOAP Component, you can also xref:modules/enabling-ws-security.adoc[enable WS-security], specify data bindings, and add interceptors to your CXF Web service. For more information about how Mule leverages the CXF framework, consult the xref:modules/cxf-module-configuration-reference.adoc[CXF Module Configuration Reference].

== Assumptions

This document describes the details of the example within the context of *Mule Studio*, Mule ESB’s graphical user interface (GUI). Where appropriate, the XML configuration accompanies the Studio interface screenshots. This document assumes that you are familiar with Mule ESB and the Mule Studio interface. To increase your familiarity with Mule Studio, consider reading xref:5@studio::index.adoc[Mule Studio Essentials].
...
+
image::soap-general.png[SOAP_general]
+
...
image::client-attribetes-together.png[]
...
+
image::interceptor-provider.png[interceptor_provider]
...
Consult the xref:modules/enabling-ws-security.adoc[Enabling WS-Security] document to configure the security elements of your Web service.
...
* Examine details about the xref:3.5@modules/cxf-module-reference.adoc[CXF Module] in Mule.
* Review a xref:3.5@xml-only-soap-web-service-example.adoc[Mule example application] which demonstrates a SOAP Web service.
* Learn more about applying xref:3.5@modules/enabling-ws-security.adoc[security] to SOAP Web services.
* Review an the xref:3.5@soap-web-service-security-example.adoc[Mule example application] which demonstrates the use of WS-security.