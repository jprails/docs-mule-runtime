...
If you're running your Mule configuration from Anypoint Studio, the log outputs are visible right in Anypoint Studio's console window. If you're running Mule from the command line, the logs then are visible in your OS console.
...
For more on how to use the Logger component in Studio, see xref:components/logger-component-reference.adoc[Logger Component Reference]

You can configure what gets logged, where it gets logged, and how by editing a configuration file that sits in your project. For a description of what you can do through this and instructions on how to set it up, see xref:debug/logging-in-mule.adoc[Logging in Mule].