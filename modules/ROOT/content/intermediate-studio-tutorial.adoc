...
|*xref:transformers/variable-transformer-reference.adoc[Variable Transformer]* |Sets a flow variable on the message that can be invoked elsewhere in the flow; find this message processor in the Transformers section of the palette
|xref:routers/choice-flow-control-reference.adoc[*Choice Router*] |Routes incoming messages according to configured logic; find this in the Flow Controls section of the palette
|*xref:transformers/set-payload-transformer-reference.adoc[Set Payload Transformer]* |Modifies your payload into a different message, depending on the results of the choice routing; find this in the Transformers section of the palette
|xref:components/logger-component-reference.adoc[*Logger*] |Logs messages or activity based on the value of a Mule expression; find this in the Components section of the palette at the right of the screen
|===
...
image::studio-logger-config-alt.png[Studio-logger-config]
...
* xref:mel/mule-expression-language-mel.adoc[complete reference]
