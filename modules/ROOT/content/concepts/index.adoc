...
image::concepts/simple-example-flow-studio.png[Simple+Example+Flow+Studio]
...
When you configure the individual elements that link together to make your flow, you can use xref:mel/mule-expression-language-mel.adoc[Mule Expression Language] to extract information about the message or its environment and instruct Mule to make processing decisions based on that information. Later in this series, you can examine how to use expressions in Mule.

== See Also

* *NEXT STEP:* Learn more about the xref:flows/elements-in-a-mule-flow.adoc[elements in a Mule flow].
* Skip ahead in this syllabus to the xref:5@studio::basic-studio-tutorial.adoc[Basic Studio Tutorial].