...
The *Encrypted Message Producer* application consists of three simple xref:3.6@flows/index.adoc[flows], each of which acts as a Web service provider which encrypts or decrypts a message before returning a response to the caller. Organized according to function, these flows serve different purposes within the application.
