...
This document assumes that you are familiar with xref:essentials-of-using-mule-esb-3.adoc[Mule ESB] and the xref:mule-studio-essentials.adoc[Mule Studio interface]. To increase your familiarity with Mule Studio, consider completing one or more xref:getting-started-with-mule-studio.adoc[Mule Studio Tutorials].
...
There are different ways to xref:3.3@mule-management-console::quick-start-guide-to-mule-esb-server-and-the-management-console.adoc[start the Mule server]:
...
The Tweetbook application uses the Mule Secure Token Service to apply Oauth 2.0 to the API it exposes. By including the OAuth2 message processor in the Web service’s flow, Tweetbook defines itself as an OAuth2 service provider and enables Mule to perform the OAuth dance with consumers. +
 +
 image:security/tweetbook-oauth2-provider.png[]
...
* For more information on Oauth 2.0 in Mule, refer to xref:3.8@security/mule-secure-token-service.adoc[Mule Secure Token Service].