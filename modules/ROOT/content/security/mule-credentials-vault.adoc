...
:page-aliases: mule-credentials-vault.adoc
...
[NOTE]
This topic introduces the idea of Global Elements; if you are unfamiliar with this functionality, access xref:3.6@global-elements.adoc[Understand Global Mule Elements] to learn more before proceeding.
...
image::security/one-one-one.png[]
...
image::security/one-one-many.png[]
...
image::security/multiple-one-one.png[]
...
+
image::security/test-properties2.png[test_properties2]
. Click the image:security/error.png[] in the `.properties` tab to close the Properties File Editor window.

. In the Package Explorer, right-click the `.properties` file, then select *Open With* > *Mule Properties Editor*.
. Click the green image:security/add.png[plus] icon in the Studio toolbar (see image below) to open the *Add a new property* dialog.
+
image::security/add-new-prop.png[add_new_prop]
+
. Add the key-value pair (property) you wish to record in the properties file.
+
image::security/add-key-value.png[add_key-value]
...
====
+
image::security/setup-encryption.png[setup_encryption]
...
+
image::security/encrypted-value.png[encrypted_value]
+
...
+
image::security/global-secure.png[global_secure]
+
...
* Access the xref:3.6@security/anypoint-enterprise-security-example-application.adoc[example application] which demonstrates Anypoint Enterprise Security in action.