...
+
. Click the image:security/error.png[error png] in the `.properties` tab to close the Properties File Editor window.
. In the Package Explorer, right-click the `.properties` file, then select *Open With* > *Mule Properties Editor*.
. Click the green image:security/add.png[(plus)] icon in the Studio toolbar (see the image below) to open the *Add a new property* dialog.
+
...
* Access the xref:security/anypoint-enterprise-security-example-application.adoc[example application] which demonstrate Anypoint Enterprise Security in action.