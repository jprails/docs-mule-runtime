...
:page-aliases: configuring-the-spring-security-manager.adoc

Use Spring Security 3.0 as a Security Manager inside of Mule. You can use any of the library's security providers such as JAAS, LDAP, CAS (Yale Central Authentication service), and DAO. For more information on the elements you can configure for a Mule security manager, see xref:3.7@security/security-manager-configuration-reference.adoc[Security Manager Configuration Reference].
