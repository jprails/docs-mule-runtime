...
:page-aliases: fips-140-2-compliance-support.adoc
...
+
image::security/fips-install-jars.png[FIPS-install-jars]
+
...
+
image::security/fips-register-jar.png[FIPS-register-jar]
+
. Adjust the numbering of the other security providers in the file so that they remain sequential.
+
image::security/fips-register-jar-renumber.png[FIPS-register-jar-renumber]
. Save.
...
+
image::security/fips-wrapper-conf-edit.png[FIPS-wrapper-conf-edit]
+
. Save.

When Mule next launches, the startup readout indicates that FIPS security mode is enabled.

image::security/fips-startup.png[FIPS-startup]
...
xref:3.6@security/anypoint-enterprise-security.adoc[Anypoint Enterprise Security] version 1.3 supports FIPS security mode. When in FIPS security mode, Mule will not register a security provider, allowing the module to rely on the platform security provider.
