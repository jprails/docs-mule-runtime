...
. Create and configure the global element through the Global Element tab at the bottom of the canvas, or directly within the CRC32 processor's *Properties* pane by clicking the image:security/add.png[(plus)] icon to the right of the *Connector Ref* field.
...
+
image::security/crc32-1.png[crc32+1]
+
[%header%autowidth.spread]
|===
|Attribute |Req'd |Value
...
. Configure the message processor's attributes according to the table below.
+
image::security/crc32-2.png[crc32+2]
+
...
. Click the image:security/add.png[(plus)] icon next to the  *Connector Configuration* field.
