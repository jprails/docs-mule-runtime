...
:page-aliases: mule-high-availability-ha-clusters.adoc
...
image::administer/clustering/cluster.png[]
...
image::administer/clustering/failovernofail.png[FailoverNoFail]
...
image::administer/clustering/failovernode2fail.png[FailoverNode2Fail]
...
image::administer/clustering/cluster-diagram.png[]
...
* Cluster lifecycle management and control. See xref:3.7@mule-management-console::managing-mule-high-availability-ha-clusters.adoc[Managing Mule High Availability (HA) Clusters].
* Cluster and node performance monitoring. See xref:3.7@mule-management-console::monitoring-a-cluster.adoc[Monitoring a Cluster].
...
image::administer/clustering/load-balancing.png[load_balancing]
...
You must use *XA transactions* to move messages between dissimilar transports that support transactions. This ensures that the Mule commits associated transactions from both transports as a single unit. See xref:transactions/transaction-management.adoc[Transaction Management] for more information on XA and other types of transactions supported by Mule.
...
[NOTE]
Note that high performance is implemented differently on xref:cloudhub::index.adoc[CloudHub], so this section applies only for on-premises deployments.
...
* If your application processes messages from a non-transactional transport, use a xref:transactions/reliability-patterns.adoc[reliability pattern] to move them to a transactional store such as a VM or JMS store.
...
* Implement xref:transactions/reliability-patterns.adoc[reliability patterns] to create high reliability applications.

== Prerequisites and Limitations

* With out of box configurations it is recommended that you scale a cluster to no more than eight Mule ESB nodes. If you need more nodes than eight, contact https://www.mulesoft.com/support-and-services/mule-esb-support-license-subscription[MuleSoft Support].  You can have at least two servers in a cluster, each of which must run in a different physical (or virtual) machine.
...
* If your xref:endpoints/endpoint-configuration-reference.adoc[custom message source] does not use a message receiver to define node http://en.wikipedia.org/wiki/Polling_(computer_science)[polling], then you must configure your message source to implement a ClusterizableMessageSource interface. +
