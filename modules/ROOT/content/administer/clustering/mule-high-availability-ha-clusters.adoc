...
* Cluster lifecycle management and control. See xref:3.8@mule-management-console::managing-mule-high-availability-ha-clusters.adoc[Managing Mule High Availability (HA) Clusters].
* Cluster and node performance monitoring. See xref:3.8@mule-management-console::monitoring-a-cluster.adoc[Monitoring a Cluster].
...
You must use *XA transactions* to move messages between dissimilar transports that support transactions. This ensures that the Mule runtime commits associated transactions from all the dissimilar transports as a single unit. See xref:transactions/transaction-management.adoc[Transaction Management] for more information on XA and other types of transactions supported by Mule runtimes.
...
High-reliability applications (ones that have zero tolerance for message loss) not only require the underlying Mule to be reliable, but that reliability needs to extend to individual connections. xref:transactions/reliability-patterns.adoc[Reliability Patterns] give you the tools to build fully reliable applications in your clusters.
...
You can also choose to define a minimum number of machines required in a cluster for it to remain in an operational state. This grants you a consistency improvement. Find more information in our xref:administer/clustering/creating-and-managing-a-cluster-manually.adoc#quorum-management[quorum management section].


== HA Cluster Demo

To evaluate Mule's HA clustering capabilities first-hand, continue on to the xref:administer/clustering/evaluating-mule-high-availability-clusters-demo.adoc[Mule HA Demo]. Designed to help new users evaluate the capabilities of Mule High Availability Clusters, the Mule HA Demo Bundle teaches you how to use the Mule Management Console to create a cluster of Mule runtimes, then deploy an application to run on the cluster. Further, this demo simulates two processing scenarios that illustrate the cluster’s ability to automatically balance normal processing load, and its ability to reliably remain active in a failover situation.
...
* xref:install/installing-an-enterprise-license.adoc[Install an Enterprise License] to start managing clusters in production.
* For instructions on how to create and manage a cluster, see xref:runtime-manager::managing-servers.adoc#create-a-cluster[Managing Servers].