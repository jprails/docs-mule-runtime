...
* Cluster lifecycle management and control. See xref:3.4@mule-management-console::managing-mule-high-availability-ha-clusters.adoc[Managing Mule High Availability (HA) Clusters].
* Cluster and node performance monitoring. See xref:3.4@mule-management-console::monitoring-a-cluster.adoc[Monitoring a Cluster].
...
* If your xref:message-sources.adoc[custom message source] does not use a message receiver to define node http://en.wikipedia.org/wiki/Polling_(computer_science)[polling], then you must configure your message source to implement a ClusterizableMessageSource interface. +
