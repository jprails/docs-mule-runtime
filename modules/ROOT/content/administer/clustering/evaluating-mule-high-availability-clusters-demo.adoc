...
:page-aliases: evaluating-mule-high-availability-clusters-demo.adoc
...
xref:3.6@administer/clustering/mule-high-availability-ha-clusters.adoc[Learn More...]
...
. *A demo application to deploy on the cluster.* +
 This *cluster-demo-app* consists of four xref:3.6@flows/index.adoc[flows]. The application receives messages from a JMS queue, adds information to the messages, then dispatches them to another JMS queue. It uses two types of transactions to safely process messages:
...
+
image::administer/clustering/widgetui-modified.png[widgetUI_modified]

== Get Started!

xref:3.7@administer/clustering/1-installing-the-demo-bundle.adoc[1 - Installing the Demo Bundle]

xref:3.6@administer/clustering/2-creating-a-cluster.adoc[2 - Creating a Cluster]

xref:3.6@administer/clustering/3-deploying-an-application.adoc[3 - Deploying an Application]

xref:3.6@administer/clustering/4-applying-load-to-the-cluster.adoc[4 - Applying Load to the Cluster]

xref:3.7@administer/clustering/5-witnessing-failover.adoc[5 - Witnessing Failover]

xref:3.7@administer/clustering/6-troubleshooting-and-next-steps.adoc[6 - Troubleshooting and Next Steps]