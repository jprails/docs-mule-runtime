...
https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/module/client/MuleClient.html[MuleClient] is a simple interface for Java clients to send and receive messages from a Mule server and other applications.
...
For SOAP support, you can use the Mule xref:modules/cxf-module-reference.adoc[CXF Module Reference].
...
This code attempts to receive a message from a mailbox called `ross` on `mail.my.org` and returns after five seconds if no message was received. Calling `request()` works for all Mule supported transports, but it is more usual to make event request calls where there is a store to be queried such as a queue, file directory, or some other repository.
...
When you need to dispatch or request events in Mule, you should use the current https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/MuleEventContext.html[org.mule.api.MuleEventContext] and call the send/dispatch/request methods on the context instead.
...
To gain access to the `MuleEventContext` inside your flows, you can implement the https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/lifecycle/Callable.html[org.mule.api.lifecycle.Callable] interface.
...
To handle situations where multiple results occur, Mule has introduced the  https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection] message type.

This type of message contains all message results in the order they were received. Note that https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection] extends https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/MuleMessage.html[org.mule.api.MuleMessage], so the interface is similar. If there are multiple results, the `MuleMessage.getPayload()` method returns a `java.util.List` containing the payloads of each of the returned messages.
...
The Mule client allows you to make synchronous calls without blocking by using the `sendAsync()`method, which returns a https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/api/FutureMessageResult.html[FutureMessageResult] that can be queried later.
