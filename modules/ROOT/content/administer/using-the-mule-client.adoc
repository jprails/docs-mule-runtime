...
:page-aliases: using-the-mule-client.adoc
...
https://www.mulesoft.org/docs/site/current/apidocs/org/mule/module/client/MuleClient.html[MuleClient] is a simple interface for Java clients to send and receive messages from a Mule server and other applications. You can use the Mule client for:
...
. The Mule URL endpoint: any valid xref:endpoints/mule-endpoint-uris.adoc[Mule Endpoint] used to determine the transport, endpoint, and other information about delivery of the message. This can also be an endpoint name stored in configuration.
...
For SOAP support, you can use the Mule xref:modules/cxf-module-reference.adoc[CXF Module Reference] or #[AXIS:Axis Transport]#. For more information about Mule Axis support, see #[AXIS:Axis Web Services and Mule]#.
...
When you need to dispatch or request events in Mule, you should use the current https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleEventContext.html[org.mule.api.MuleEventContext] and call the send/dispatch/request methods on the context instead.

To gain access to the `MuleEventContext` inside your services, you can implement the https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/lifecycle/Callable.html[org.mule.api.lifecycle.Callable] interface.
...
Some outbound routers such as the xref:outbound-routers.adoc[List Message Splitter], xref:outbound-routers.adoc[Multicaster], and xref:outbound-routers.adoc[Recipient List] may return more that one result message in the following cases:
...
To handle situations where multiple results occur, Mule has introduced a new message type https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection]. This type of message contains all message results in the order they were received. Note that https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessageCollection.html[org.mule.api.MuleMessageCollection]extends https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/MuleMessage.html[org.mule.api.MuleMessage], so the interface is similar. If there are multiple results, the `MuleMessage.getPayload()` method returns a `java.util.List` containing the payloads of each of the returned messages.
...
The Mule client allows you to make synchronous calls without blocking by using the `sendAsync()` method, which returns a https://www.mulesoft.org/docs/site/current/apidocs/org/mule/api/FutureMessageResult.html[FutureMessageResult] that can be queried later.
...
If you do not set the wire format, the serialization format is used. For more information on transformers, see xref:transformers/using-transformers.adoc[Using Transformers].
