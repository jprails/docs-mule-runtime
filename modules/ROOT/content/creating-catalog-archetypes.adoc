...
xref:pattern-based-configuration.adoc[Configuration patterns] are elements that simplify configuring specific and recurring activities. Mule comes with some xref:configuration-patterns.adoc[ready made patterns] that cover generic scenarios.
...
The module archetype will xref:3.6@maven/creating-module-archetypes.adoc[ask various questions] and then generate the files. You can also use this plug-in without user prompts by entering all the arguments at the command line. For a full list of arguments that can be passed in, see the Command Line Options.
...
* *siso*: The pattern receives messages from a single inbound endpoint and dispatches to a single outbound endpoint. The xref:bridge-pattern.adoc[Bridge], xref:validator-pattern.adoc[Validator], and xref:3.6@api-usage/proxying-web-services.adoc[Web Service Proxy] patterns are of this kind.
