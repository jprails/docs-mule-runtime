...
The xref:transports/jdbc-transport-reference.adoc[JDBC connector] — a global element in the JDBC example application — helps the JDBC Endpoint with its tasks (listed above).

. As a xref:transports/connecting-using-transports.adoc[connector], it helps the JDBC endpoint access the database by providing specific details about how to connect to it, such as where it exists (the URL), and how to gain access to it (username and password for database). See image below, left.
...
The next building block in the flow, the **xref:transformers/property-transformer-reference.adoc[Property Transformer]**, sets a property on the message payload to specify the type of content it contains (for HTTP protocol). In this case, the `Set Property` property transformer sets the value of the property to `text/xml`.
...
Notice that the CSVLoader flow also contains a **xref:exceptions/rollback-exception-strategy.adoc[Rollback Exception Strategy]**.
