...
:page-aliases: batch-filters-and-batch-commit.adoc
...
This document assumes that you are familiar with xref:6@studio::index.adoc[Anypoint Studio], have reviewed the overview and basic anatomy of xref:batch/batch-processing.adoc[Batch Processing], and that you understand how batch jobs handle xref:batch/batch-processing.adoc[records which fail processing]. You should also consider xref:batch/batch-streaming-and-job-execution.adoc#streaming-batch-commits[Streaming Batch Commits].
...
image::batch/accept-expression.png[accept_expression]
...
image::batch/accept-policy.png[accept_policy]
...
* Batch filters only apply to batch steps which, in turn, are only usable xref:batch/batch-processing.adoc#process[within the batch process phase] of a batch job. You cannot apply filters with the Input or On Complete phases.
...
*Note:* See also xref:batch/batch-streaming-and-job-execution.adoc#streaming-batch-commits[Streaming Batch Commits].
...
image::batch/aggregator.png[]

image::batch/batch-commit.png[batch_commit]
...
* Batch Commit scopes can only exist in batch steps which, in turn, are only usable xref:batch/batch-processing.adoc#process[within the batch process phase] of a batch job. You cannot use batch commits within the Input or On Complete phases.
...
In previous versions of Mule, the commit block exposed only the grouped record’s payload and didn't allow you to change those payloads nor to retrieve the associated xref:batch/record-variable.adoc[record variables].
...
During the xref:batch/batch-processing.adoc#process[process phase] of your batch job, you can replace, change, or store the payload data.

By adding a xref:scopes/foreach.adoc[Foreach Scope] you can iterate trough a fixed size commit block, and use the xref:components/expression-component-reference.adoc[Expression Component Reference] to modify the payload and create a record variable for each collected record.
...
* xref:batch/batch-processing.adoc[Batch Processing] for a full description of the steps the batch job takes in each phase of processing.
* https://www.mulesoft.com/exchange#!/import-leads-into-salesforce[Example: Import Leads Into Salesforce].
...
image::batch/example-batch2.png[example_batch2]
...
* Read about the xref:batch/batch-processing.adoc[basic anatomy] of batch processing in Mule.
