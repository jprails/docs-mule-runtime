...
:page-aliases: batch-job-instance-id.adoc
:keywords: connectors, anypoint, studio, batch, batch processing

Users of xref:batch/batch-processing.adoc[Batch] processing frequently need the ability to determine a Batch job's instance ID during the execution phases of a Batch job.
...
image::batch/batchjobproperties.png[batchJobProperties]
