...
* Access the xref:batch/batch-processing.adoc[full documentation for Batch Processing] for the latest general availability release of Mule.
* For more information on best design-time practices with DataSense and DataMapper, consult the xref:6@studio::datasense/datasense.adoc[DataSense documentation].