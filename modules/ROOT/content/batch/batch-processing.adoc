...
:page-aliases: batch-processing.adoc
...
image::batch/batch-main1.png[batch_main1]
...
To build applications with Batch jobs and run them in a test or production environment, you must build the application with an Enterprise runtime. Learn more about xref:6@studio::mule-runtime/changing-runtimes-in-studio.adoc[Changing Runtimes in Studio].
====
...
* Anypoint Studio. See xref:6@studio::index.adoc[Anypoint Studio Essentials] chapter to learn more about developing with Mule's graphical user interface.
* xref:3.8@connectors::index.adoc[Anypoint Connectors].
* xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper User Guide and Reference]
...
* xref:6@studio::datasense/datasense.adoc[DataSense].
* xref:scopes/poll-reference.adoc[Polls].
...
image::batch/batch-main3.png[batch_main3]
...
When it comes to a batch job, all records are first stored in a persistent queue before the <<process,processing phase>> begins, so the traditional threading model wouldn't apply.
...
Access xref:batch/batch-processing-reference.adoc[Batch Processing Reference] for more information.
====
...
* A *batch job instance* is an occurrence in a Mule application whenever a Mule flow executes a batch job. Mule creates the batch job instance in the <<Load and Dispatch>> phase. Every _batch job instance_ is identified internally using a unique String known as xref:batch/batch-job-instance-id.adoc[batch job instance id].
====
...
Since Batch was designed to work with millions of records, they need to be stored in a persistent storage queue. However, consider having 1 million records to place in a queue for a 3 step batch job. At least three million I/O operations occur as Mule takes and requests each record as it moves through the <<batch-processing-phases,job's phases>>.
...
image::batch/batch-phases.png[batch_phases]
...
image::batch/input-phas.png[input_phas]
...
. Mule sends the message payload through a xref:routers/splitter-flow-control-reference.adoc[collection splitter]. This first step triggers the creation of a new batch job instance.
...
image::batch/batch-diagram.jpg[batch+diagram]

Beyond simple processing of records, there are several things you can do with records within batch steps:

* You can set *record variables* on records and pass them from step to step (xref:batch/record-variable.adoc[read more])

* You can apply filters by adding *accept expressions* within each batch step to prevent the step from processing certain records; for example, you can set a filter to prevent a step from processing any records which failed processing in the preceding step (xref:batch/batch-filters-and-batch-commit.adoc[read more])

* You can *commit* records in groups, sending them as bulk upserts to external sources or services. (xref:batch/batch-filters-and-batch-commit.adoc[read more])
...
image::batch/on-complete-phase.png[on-complete_phase]
...
+
image::batch/batch-main.png[batch_main]
+
...
image::batch/trigger-ref1.png[trigger_ref1]
...
image::batch/trigger-source.png[trigger_source]
...
Read more about xref:batch/batch-filters-and-batch-commit.adoc[fine-tuning filters] on batch steps to manage failed records at a more granular level.


Also check xref:batch/using-mel-with-batch-processing.adoc#mel-functions-that-involve-batch-jobs[MEL expressions for Batch Processing] for a detailed description of the MEL expressions you can use to monitor
...
If a batch job _does not_ accumulate enough failed records to cross the `max-failed-records` threshold, _all_ records – successes and failures – continue to flow from batch step to batch step; use xref:batch/batch-filters-and-batch-commit.adoc[filters] to control which records each batch step processes.
...
[TIP]
Note that this example introduces features not discussed in great detail in this document. Consult xref:batch/batch-filters-and-batch-commit.adoc[Batch Filters and Batch Commit] and xref:batch/record-variable.adoc[Record Variable] for more information.
...
image::batch/example-batch.png[example_batch]
...
. The application first uses a xref:3.8@connectors::file-connector.adoc[File connector] to upload a CSV file, then uses a xref:dataweave/index.adoc[DataWeave] to convert the data format into a collection (see mapping below). Each item in the collection represents a lead. Each lead contains a company name, a first name, a last name, a birthday and an email address.
+
image::batch/example-mapping.png[example_mapping]
...
[start=3]
. Mule begins processing each lead as a record. The first batch step, `lead-check`, uses a *Salesforce Connector* wrapped with a xref:scopes/message-enricher.adoc[Message Enricher] to:
...
image::batch/example-query3.png[example_query3]

[discrete.view]
=== XML Editor

image::batch/query4.png[]
=====
...
image::batch/example-filter3.png[example_filter3]

[discrete.view]
=== XML Editor

image::batch/batch-example-filter.png[]
=====
...
image::batch/example-insert1.png[example_insert1]

[discrete.view]
=== XML Editor

image::batch/example-insert2.png[example_insert2]
=====
...
* Batch processing does not support the use of xref:mule-configuration/business-events.adoc[Business Events].
* xref:cloudhub::insight.adoc[Insight] does not support visibility into batch processing.
* xref:3.8@mule-management-console::index.adoc[Mule Management Console (MMC)] does not support visibility into batch processing.
...
* Access xref:batch/batch-processing-reference.adoc[reference details] about batch processing.
* Examine the xref:batch/batch-processing-reference.adoc[attributes] you can configure for batch jobs, steps and message processors.
* Learn more about xref:batch/batch-filters-and-batch-commit.adoc[filters] in batch processing.
* Learn more about xref:batch/batch-filters-and-batch-commit.adoc[batch commit].
* Learn more about setting and removing xref:batch/record-variable.adoc[record-level variables].
* Learn more about xref:batch/batch-processing-reference.adoc[MEL expressions] you can use to access batch processing data.
* Read more about xref:cloudhub::cloudhub-fabric.adoc[Runtime Manager support for batch processing].
* Learn more about xref:3.8@connectors::index.adoc[Anypoint Connectors].
* Learn more about xref:scopes/poll-reference.adoc[Polling and Watermarks].
