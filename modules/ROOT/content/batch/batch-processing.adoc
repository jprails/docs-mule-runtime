...
A batch job executes when triggered by either a *batch executor* in a Mule flow or a *message source* in a batch-accepting input; when triggered, Mule creates a new *batch job instance*. When all records have passed through all batch steps, the batch job instance ends and the *batch job result* can be summarized in a report to indicate which records succeeded and which failed during processing. +
 +
Access xref:3.6@batch/batch-processing-reference.adoc[Batch Processing Reference] for more information.
...
* you can set *record variables* on records and pass them from step to step (xref:3.6@batch/record-variable.adoc[read more])
* you can apply filters by adding *accept expressions* within each batch step to prevent the step from processing certain records; for example, you can set a filter to prevent a step from processing any records which failed processing in the preceding step (xref:3.6@batch/batch-filters-and-batch-commit.adoc[read more])
* you can **commit** records in groups, sending them as bulk upserts to external sources or services. (xref:3.6@batch/batch-filters-and-batch-commit.adoc[read more])
...
Read more about xref:3.6@batch/batch-filters-and-batch-commit.adoc[fine-tuning filters] on batch steps to manage failed records at a more granular level.
...
If a batch job _does not_ accumulate enough failed records to cross the `max-failed-records` threshold, _all_ records – successes and failures – continue to flow from batch step to batch step; use xref:3.6@batch/batch-filters-and-batch-commit.adoc[filters] to control which records each batch step processes.
...
[TIP]
Note that this example introduces features not discussed in great detail in this document. Consult xref:3.6@batch/batch-filters-and-batch-commit.adoc[Batch Filters and Batch Commit] and xref:3.6@batch/record-variable.adoc[Record Variable] for more information.
...
. Mule begins processing each lead as a record. The first batch step, **`lead-check`**, uses a *Salesforce Connector* wrapped with a xref:3.6@scopes/message-enricher.adoc[Message Enricher] to: +
+
...
image::batch/example-filter4-1.png[example_filter4+(1)]
=====
...
* Batch processing does not support the use of xref:3.6@mule-configuration/business-events.adoc[Business Events].
...
* Access xref:3.6@batch/batch-processing-reference.adoc[reference details] about batch processing.
* Examine the xref:3.6@batch/batch-processing-reference.adoc[attributes] you can configure for batch jobs, steps and message processors.
* Learn more about xref:3.6@batch/batch-filters-and-batch-commit.adoc[filters] in batch processing.
* Learn more about xref:3.6@batch/batch-filters-and-batch-commit.adoc[batch commit].
* Learn more about setting and removing xref:3.6@batch/record-variable.adoc[record-level variables].
* Learn more about xref:3.6@batch/batch-processing-reference.adoc[MEL expressions] you can use to access batch processing data.
...
* Learn more about xref:3.6@scopes/poll-reference.adoc[Polling and Watermarks].
