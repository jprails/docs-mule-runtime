...
This document assumes that you are familiar with Mule ESB and the xref:5@studio::index.adoc[Mule Studio interface]. To increase your familiarity with Studio, consider completing one or more xref:5@studio::basic-studio-tutorial.adoc[Mule Studio Tutorials]. Further, this example assumes you are familiar with XML coding and that you have a basic understanding of xref:flows/index.adoc[Mule flows] and SOAP as a Web service paradigm and the practice of WSDL-first Web service development.
...
The first building block in the orderService flow, an xref:http-endpoint-reference.adoc[HTTP Inbound Endpoint], receives orders entered by the user in the Web page served by the application. A xref:soap-component-reference.adoc[SOAP Component] converts the incoming XML into the http://en.wikipedia.org/wiki/JAXB[JAXB annotated classes] referenced in the Web service interface. The xref:routers/choice-flow-control-reference.adoc[Choice Flow Control] in the flow parses the message payload; if the payload defines the manufacturer as `Samsung`, the Choice Strategy routes the message to a xref:transports/vm-transport-reference.adoc[VM Outbound Endpoint] which calls the samsungOrder flow. If the payload defines the manufacturer as `Default`, the Choice Strategy routes the message to a VM Outbound Endpoint which calls the inhouseOrder flow.
...
* We use a xref:filters/message-filter.adoc[Message Filter] to throw an exception if the HTTP response code is anything other than 200 (success). Without it, the application would consider any HTTP response as successful, including errors such as a SOAP fault
...
The message source for this flow is a xref:transports/jms-transport-reference.adoc[JMS Inbound Endpoint]. The flow immediately initializes the variable `Price`, then assigns to it the value returned by the in-house priceService flow. The inhouseOrder flow then saves this value to the company database. The session variable `totalValue` holds the total price of this item. The last building block in the flow, a xref:components/groovy-component-reference.adoc[Groovy Component], creates a purchase receipt with the relevant information.
...
This flow is sourced by the HTTP Inbound Endpoint followed by a SOAP Component configured as a JAX-WS Service. The service implementation is in the `Samsung Service Impl`, a xref:transformers/java-transformer-reference.adoc[Java Component].
...
* Learn more about the xref:routers/choice-flow-control-reference.adoc[Choice Router/Choice Flow Control].
* Learn more about the xref:transports/vm-transport-reference.adoc[VM] and xref:transports/jms-transport-reference.adoc[JMS] transports.
