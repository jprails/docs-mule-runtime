...
The first building block in the orderService flow, an xref:3.5@connectors::http-connector.adoc[HTTP Inbound connector], receives orders entered by the user in the Web page served by the application. A xref:components/cxf-component-reference.adoc[CXF Component] converts the incoming XML into the http://en.wikipedia.org/wiki/JAXB[JAXB annotated classes] referenced in the Web service interface. The xref:routers/choice-flow-control-reference.adoc[Choice Router] in the flow parses the message payload; if the payload defines the manufacturer as `Samsung`, the Choice Strategy routes the message to a xref:transports/vm-transport-reference.adoc[VM Outbound connector] which calls the samsungOrder flow. Otherwise, the Choice Strategy routes the message to a xref:transports/jms-transport-reference.adoc[JMS Outbound connector] which calls the inhouseOrder flow.
...
* The application uses a xref:filters/message-filter.adoc[Message Filter] to throw an exception if the HTTP response code is anything other than 200 (success). Without it, the application would consider any HTTP response as successful, including errors such as a SOAP fault.
...
The message source for this flow is a xref:transports/jms-transport-reference.adoc[JMS Inbound connector]. The flow immediately initializes the flow variable `price`, then assigns to it the value returned by the in-house priceService flow. The inhouseOrder flow then saves this value to the company database using the xref:3.5@connectors::database-connector.adoc[Database Connector]. The session variable `totalValue` holds the total price of this item. The last building block in the flow, a xref:components/groovy-component-reference.adoc[Groovy Component], creates a purchase receipt with the relevant information.
...
* The `Redelivery exhausted` configuration allows us to determine what to do when the number of retries has reached the maximum specified in the `maxRedeliveryAttempts` attribute of the xref:exceptions/rollback-exception-strategy.adoc[Exception Strategy].
* Mule caches the price returned by the priceService flow in an in-memory xref:modules/mule-object-stores.adoc[Object Store]. The key to the store is the ID of the product requested. The first time that a given product ID appears, the `Enrich with price` xref:scopes/message-enricher.adoc[Message Enricher] invokes the priceService to obtain the price for the product. After that, the flow uses the cached value for the product.
...
This flow is sourced by the HTTP Inbound connector followed by a CXF Component configured as a JAX-WS Service. The service implementation is in the `Samsung Service Impl`, a xref:transformers/java-transformer-reference.adoc[Java Component].
...
* Learn more about the xref:routers/choice-flow-control-reference.adoc[Choice Router].
* Learn more about the xref:transports/vm-transport-reference.adoc[VM] and xref:transports/jms-transport-reference.adoc[JMS] connectors.
* Learn more about the xref:3.5@connectors::database-connector.adoc[Database Connector].
* Learn more about xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper].
* Learn more about the xref:scopes/cache-scope.adoc[Cache] and xref:scopes/foreach.adoc[Foreach] scopes.