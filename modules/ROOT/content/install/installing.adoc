...
. xref:install/downloading-and-starting-mule-esb.adoc[Download, then launch Mule.]
. xref:5@studio::mule-runtime/adding-community-runtime.adoc[Add Community runtime to AnypointStudio] (_optional_).
. xref:install/installing-an-enterprise-license.adoc[Install an Enterprise license] (_optional_).
. xref:5@studio::install-update-items-in-studio.adoc[Extend Mule with plugins, modules, runtimes and connectors] (_optional_).

[cols="34,33,33"]
|===
|image:install/anypoint-studio.png[anypoint_Studio]  |image:install/mule-enterprise.png[mule_enterprise] | image:install/mule-community.png[mule_community]
