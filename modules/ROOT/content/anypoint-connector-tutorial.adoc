...
After learning some of the basic functionality in the xref:5@studio::basic-studio-tutorial.adoc[Basic] and xref:general:getting-started:content-based-routing.adoc[Content-Based Routing], it's time to try your hand at developing a more complex application which makes use of **Anypoint Connectors**.
...
This tutorial assumes that you have xref:6@studio::download-and-launch-anypoint-studio.adoc[downloaded and installed Mule Studio]. If you do not have any previous experience with Eclipse or an Eclipse-based IDE, please review the brief introduction to the xref:5@studio::index.adoc[Mule Studio interface] and the document describing xref:global-elements.adoc[Global Elements in Mule]. Further, consider following the xref:5@studio::basic-studio-tutorial.adoc[Basic] and xref:general:getting-started:content-based-routing.adoc[Content-Based Routing] tutorials.
...
However, if you wanted to expose such an application publicly, you would need to configure your application to xref:3.4@connectors::using-a-connector-to-access-an-oauth-api.adoc[authenticate via OAuth].
====
+
...
Launch Mule Studio and create a new Mule Project named `Connector Tutorial`. If you need more directions on launching Studio and creating a project, please refer to the xref:5@studio::basic-studio-tutorial.adoc[Basic Studio Tutorial].
...
|*xref:scopes/foreach.adoc[Foreach scope]* |processes each item in a POJO iteratively
|*xref:components/logger-component-reference.adoc[Logger component]* |logs messages or activity based on the value of a Mule expression; find this in the Components section of the palette at the right of the screen
|===
...
=== image:icon-question-blue-big.png[] Hints
...
There are a few ways to accomplish this task, one of which is to add the extra text to the tweet inside a new variable. However, you can take a shortcut using the xref:transformers/set-payload-transformer-reference.adoc[set payload] transformer. Replace the content of the entire payload using an expression composed of multiple parts, such as the following the following:
...
* Import xref:3.4@connectors::installing-connectors.adoc[additional connectors] into your instance of Studio.