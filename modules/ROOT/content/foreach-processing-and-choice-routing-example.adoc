...
This document assumes that you are familiar with Mule ESB and the xref:5@studio::index.adoc[Mule Studio interface]. To increase your familiarity with Studio, consider completing one or more xref:5@studio::basic-studio-tutorial.adoc[Mule Studio Tutorials].
...
Then, Mule employs a content-based router to direct the message for further processing. The **xref:routers/choice-flow-control-reference.adoc[Choice Flow Control] **routes each message to one of two processing pathways according to its payload contents (see image and code below).
