...
This example application consists of several xref:flows/index.adoc[flows and subflows]. These flows integrate Web services to process end user requests for interest rate quotes from banks.
...
The request-response **xref:3.5@connectors::http-connector.adoc[HTTP Inbound connector] **in this flow receives an end user request. Because it has a request-response exchange pattern, this HTTP connector is responsible for both receiving and returning messages.
...
Then, Mule employs a content-based router to direct the message for further processing. The **xref:routers/choice-flow-control-reference.adoc[Choice Router] **routes each message to one of two processing pathways according to its payload contents (see image and code below).
...
The `Enrich with creditProfile` enricher contains only a **xref:flows/flow-reference-component-reference.adoc[Flow Reference Component]**. This type of component invokes other flows, or subflows, in the application to acquire, then add data to the message. In this case, the `lookupCustomerCreditProfile` component demands that the lookupCustomerCreditProfile subflow access an external Web service to acquire the borrower’s credit score. Mule enriches the message with the credit score, then passes the message to the next enricher in the flow.
...
Mule leverages http://cxf.apache.org/[Apache’s CXF framework] to build Web services. The Processor Chain that wraps the **xref:components/cxf-component-reference.adoc[CXF Component]** and HTTP outbound connector is a CXF requirement. It ensures that Mule completes all processing activities prior to logging the processing result.
...
* For more information on routing messages, see xref:routers/choice-flow-control-reference.adoc[Choice Router].
* For more information on enriching messages, see xref:scopes/scopes.adoc[Scopes].
* For more information on setting variables on messages, see the xref:transformers/variable-transformer-reference.adoc[Variable Transformer Reference].
...
* For more information on configuring a CXF component, see xref:components/cxf-component-reference.adoc[CXF Component Reference].
