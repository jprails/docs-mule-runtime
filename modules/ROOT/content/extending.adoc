...
* A *module* is a package of related functionality in Mule, such as the XML module, which provides XML-based utilities like filters and routers. For a list of the available modules you can use, see xref:3.7@modules/modules-reference.adoc[Modules Reference].

* A *xref:3.7@transports/connecting-using-transports.adoc[transport]* is a type of module that carries messages between Mule flows via a specific protocol. For a list of the available transports you can use, see xref:3.6@transports/transports-reference.adoc[Transports Reference].
...
When working with transports, note that you can xref:3.7@transports/configuring-a-transport.adoc[configure an existing transport], or you can xref:3.6@transports/creating-transports.adoc[create a new one]. The recommended approach is to try to use and configure an existing transport first.

* xref:3.7@components/extending-components.adoc[Extending Components]
* xref:3.7@modules/custom/custom-message-processors.adoc[Custom Message Processors]
* xref:3.6@maven/creating-example-archetypes.adoc[Creating Example Archetypes]
* xref:3.6@mule-configuration/creating-a-custom-xml-namespace.adoc[Creating a Custom XML Namespace]
* xref:3.6@maven/creating-module-archetypes.adoc[Creating Module Archetypes]
* xref:creating-catalog-archetypes.adoc[Creating Catalog Archetypes]
* xref:3.6@mule-configuration/creating-project-archetypes.adoc[Creating Project Archetypes]
* xref:3.6@transports/creating-transports.adoc[Creating Transports]
* xref:3.6@routers/creating-custom-routers.adoc[Creating Custom Routers]
