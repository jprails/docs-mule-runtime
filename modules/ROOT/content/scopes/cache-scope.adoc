...
[NOTE]
The default caching strategy uses an InMemoryObject store and should only be used for testing; on actual implementations see <<createcachingstrategy,Creating a Caching Strategy>>.
...
For details on configuring caching strategies, see <<createcachingstrategy,Creating a Caching Strategy>>.
...
+
image::scopes/cache-scope.png[cache_scope]
+
...
.2+|*Caching strategy reference* |Use Default caching strategy |(Default) Select if you want the cache scope to follow Mule’s <<defcachgstrat,Default Caching Strategy>>. |n/a
|Reference to a strategy |Select to configure the cache scope to follow a global caching strategy that you have created; select the global caching strategy from the drop-down menu or create one by clicking the image:scopes/add.png[(plus)]. |`cachingStrategy-ref="Caching_Strategy"`
...
|*cachingStrategy-ref* |  |(Optional) Specify the name of the global caching strategy that you have defined as a global element. If no `cachingStrategy-ref` is defined, Mule uses the <<defcachgstrat,Default Caching Strategy>>.
|===
...
. In the Cache properties editor, click radio button next to the *Reference to a strategy field*, then click image:scopes/add.png[(plus)].
...
|*custom-object-store*
.4+|See <<cfgobj,Configuring an Object Store for Cache>>, below
|*in-memory-store*
|*managed-store*
|*simple-text-file-store*
|===
...
By default, Mule stores all cached responses in an InMemoryObjectStore. <<createcachingstrategy,Creating a Caching Strategy>> and define a new xref:modules/mule-object-stores.adoc[object store] if you want to customize the way Mule stores cached responses.
...
+
image::scopes/cache-flow-2.png[cache+flow+2]
...
image::scopes/reqa.png[reqA]

image::scopes/reqb.png[reqB]

image::scopes/reqc.png[reqC]

image::scopes/reqd.png[reqD]
...
* Read more about xref:scopes/scopes.adoc[Scopes] in Mule
