...
You can make the source a namespace by simply populating the field with a xref:mel/mule-expression-language-mel.adoc[Mule Expression Language MEL] expression, for example with an xpath expression that takes arguments from the Mule message.

You can find information on using a variable with any enricher listed on these pages:

* xref:non-mel-expressions/non-mel-expressions-configuration-reference.adoc[Default enrichers] are loaded at runtime.
...
* API documents for https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/enricher/MessageEnricher.html[org.mule.core.enricher.MessageEnricher].