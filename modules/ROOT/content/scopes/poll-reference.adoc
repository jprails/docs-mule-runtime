...
:page-aliases: poll-reference.adoc
:keywords: anypoint studio, poll scope, polling, quartz, schedule, intervals, timing, trigger

While some connectors, such as xref:3.8@connectors::http-connector.adoc[HTTP] and xref:3.8@connectors::ftp-connector.adoc[FTP], utilize a polling process to actively retrieve messages from an external resource, most message processors in Mule are triggered when called by a previous element in a flow. If you want to arrange for a message processor to actively call a resource at regular intervals, use a Poll scope.

[TIP]
For greater flexibility, you can also configure your Poll scope to use a Cron expression, see xref:scopes/poll-schedulers.adoc[Poll Scheduler].

*<<Complete Code Example>>*

== Prerequisites

This document assumes that you are familiar with the xref:6@studio::index.adoc[Visual Editor in Anypoint Studio] that you can use to create applications in Studio. Further, this document references xref:global-elements.adoc[Global Elements] and the category of message processors known as xref:scopes/scopes.adoc[scopes].  Review xref:6@studio::index.adoc[Anypoint Studio Essentials] to learn more about developing with Mule's graphical user interface. Also review xref:2.0@exchange::exchange1.adoc#accessing-the-exchange-s-public-content[Anypoint Exchange] for how to locate and install a connector using Exchange.
...
+
image::scopes/poll-scope.png[poll_scope]
+
...
+
image::scopes/poll-connector.png[poll_connector]
+
. Configure the Twitter Connector:
.. Click the green plus sign to the right of *Connector Configuration*:
+
image::scopes/poll-twitter-properties.png[]
+
...
+
image::scopes/poll-twitter-config.png[]
+
** Consumer Key
** Consumer Secret
** Access Token
...
+
image::scopes/twitter.png[Twitter connector properties settings]
+
. Add more message processors to your flow to perform specific business logic after polling for data. The example below uses a xref:components/logger-component-reference.adoc[logger] component. This logger uses the xref:mel/mule-expression-language-mel.adoc[MEL] expression `#[payload]` to log the message payload collected by the Twitter connector every 1000 milliseconds.
+
image::scopes/poll-flow.png[poll_flow]
...
|*message* |string or xref:mel/mule-expression-language-mel.adoc[Mule expression] |The content that logs to console. In this case, the Mule Expression `#[payload]` outputs the message payload.
...
+
image::scopes/poll-debug.png[poll_debug]
...
+
image::scopes/poll-startstop.png[poll_startStop]
...
image::scopes/watermark-expbasic.png[]
...
image::scopes/watermark-w-selectors.png[]
...
+
image::scopes/poll-twitter-flow.png[]
+
...
+
image::scopes/poll-since-id-attribute.png[]
+
. Select the *poll* scope, and edit its properties according to the table below.
+
image::scopes/watermark-enable.png[watermark_enable]
+
[%header,cols="20a,80a"]
|===
|Attribute |Description
...
image::scopes/watermark-expcomplex.png[]

== Using Watermarks with Auto-Paging

Any connector which is enabled for xref:3.8@connectors::auto-paging-in-anypoint-connectors.adoc[auto-paging] allows you to process large data sets in separate batches. This capability mitigates for memory overload, but also imposes certain conditions when used in conjunction with watermarks. The following example illustrates the recommended method for using watermarks when polling a connector that auto-pages its response.
...
* Learn how to configure a xref:scopes/poll-schedulers.adoc[polling schedule].
...
* Learn more about xref:components/logger-component-reference.adoc[Logger].
* Learn more about xref:3.8@connectors::index.adoc[Anypoint Connectors] and xref:3.8@connectors::auto-paging-in-anypoint-connectors.adoc[auto-paging].
* Learn more about xref:flows/flow-processing-strategies.adoc[flow processing strategies].
* xref:object-store::object-store-module-reference.adoc[Object Store Module Reference]