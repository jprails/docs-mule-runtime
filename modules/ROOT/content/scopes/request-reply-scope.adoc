...
:page-aliases: request-reply-scope.adoc
...
This document assumes that you understand the difference between xref:3.7@flows/flows-and-subflows.adoc[synchronous and asynchronous flows]. Furthermore, you're also familiar with xref:3.6@scopes/scopes.adoc[Scopes] in general.
...
image::scopes/request-reply-scope-1.png[request+reply+scope+1]
...
+
image::scopes/req-rep-config.png[req_rep_config]
+
[%header,cols="4*"]
|===
...
+
image::scopes/request-response-2.png[request+response+2]
...
* Learn more about xref:3.7@flows/flows-and-subflows.adoc[synchronous and asynchronous flows].