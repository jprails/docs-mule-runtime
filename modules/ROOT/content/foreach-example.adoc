...
Mule uses a **xref:scopes/foreach.adoc[Foreach]** message processor to extract, and iteratively process, records from a database. Each record in the database contains information about a car in an inventory. However, a record does not indicate a car's manufacture century; it contains only the make, model and year of manufacture of a car. Mule must iteratively examine the message payload of each record, add information to the payload to indicate each car’s manufacture century, then return a response to the end user.
...
The *Foreach* example application contains two xref:flows/index.adoc[flows].
...
Mule then subjects the message to a **xref:scopes/foreach.adoc[Foreach]** iterative processor. With the help of a **xref:routers/choice-flow-control-reference.adoc[Choice Flow Control]**, foreach identifies which vehicles are 20th century cars, and which are 21st century cars.
...
As in Process flow, the request-response **xref:http-endpoint-reference.adoc[HTTP Inbound Endpoint] **in the Populate flow receives the end user request. The message then moves to a **xref:components/groovy-component-reference.adoc[Groovy Script Component]** which creates a table in a database, and populates the table with information about cars (see script, below).
...
image::global-elements-tab-alt.png[global_elements_tab]
