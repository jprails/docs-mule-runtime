...
For reference to the configuration of each filter, see xref:filters/filters-configuration-reference.adoc[Filters Configuration Reference].
...
You can also use the xref:modules/xml-module-reference.adoc[XPath Filter] from the xref:modules/xml-module-reference.adoc[XML Module Reference], which supports some additional properties. Mule also provides a xref:modules/xml-module-reference.adoc[Jaxen XPath Filter] based on the http://jaxen.codehaus.org[Jaxen] library that may provide faster performance in some scenarios.
...
You can also use the JXPath filter from the xref:modules/xml-module-reference.adoc[XML Module Reference], which supports some additional properties.
...
Several Mule transports and modules provide their own filters. For example, the xref:modules/xml-module-reference.adoc[XML Module Reference] includes a filter to determine if a message is XML. For more information, see xref:transports/transports-reference.adoc[Transports Reference] and xref:modules/modules-reference.adoc[Modules Reference]. Also, there are additional filters on https://www.mulesoft.com/exchange#!/?searchTerm=filters[Anypoint Exchange].

== Creating Custom Filters

The standard filters handle most filtering requirements, but you can also create your own filter. To create a filter, implement the https://www.mulesoft.org/docs/site/3.0.0/apidocs/org/mule/api/routing/filter/package-summary.html[Filter interface], which has a single method:
