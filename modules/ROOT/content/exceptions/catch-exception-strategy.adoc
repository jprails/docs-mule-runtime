...
|*Execute When* a|A MEL expression to indicate the kind of exception the catch exception strategy handles when it is embedded within a xref:exceptions/choice-exception-strategy.adoc[Choice Exception Strategy]
...
You can create one or more xref:error-handling.adoc#creating-a-global-default-exception-strategy[global exception strategies] to reuse in flows throughout your entire Mule application. First, create a Mule Configuration File and add to it a catch exception strategy. Add a xref:exceptions/reference-exception-strategy.adoc[Reference Exception Strategy] to a flow to apply the error handling behavior of your new global catch exception strategy.
