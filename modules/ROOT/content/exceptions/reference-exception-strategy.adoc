...
+
image::exceptions/draf-ref.png[draf_ref]
...
+
image::exceptions/ref-es-2.png[ref_ES_2]
...
[NOTE]
====
You can create a global exception strategy (i.e. access the Choose Global Type panel) from the reference exception strategy’s pattern properties panel. Click the (plus) button next to the Global Exception Strategy drop-down and follow the steps to create a global xref:exceptions/catch-exception-strategy.adoc[catch], xref:exceptions/rollback-exception-strategy.adoc[rollback], or xref:exceptions/choice-exception-strategy.adoc[choice] exception strategy.
