...
. The catch exception strategy replaces the message payload with new content. Mule uses the xref:mel/mule-expression-language-mel.adoc[Mule Expression Language MEL] `exception` variable to include an error number in the message payload.
