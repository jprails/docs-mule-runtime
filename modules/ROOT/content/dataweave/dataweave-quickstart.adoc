...
:page-aliases: dataweave-tutorial.adoc, dataweave-quickstart.adoc
...
[TIP]
For advanced examples that involve writing DataWeave code, see xref:dataweave/dataweave-examples.adoc[DataWeave Examples].
...
. Drag an xref:3.8@connectors::http-connector.adoc[HTTP Connector] from the palette onto your empty canvas, followed by a *Transform Message* component, then another HTTP Connector.
+
image::dataweave/dataweave-quickstart-5a4cb.png[]
...
+
image::dataweave/dataweave-quickstart-a56f0.png[]
...
+
image::dataweave/dataweave-quickstart-702fd.png[]
. Click the *Edit* icon image:dataweave/edit-button.png[edit] that appears next to the dropdown.
+
image::dataweave/dataweave-quickstart-8ea02.png[]
+
. Click the green plus sign to *Add* a new metadata type.
+
image::dataweave/dataweave-quickstart-c8ae5.png[]
+
. Name the metadata type `XML-order-input` and click *Create type*.
+
image::dataweave/dataweave-quickstart-22a00.png[]
+
...
+
image::dataweave/dataweave-quickstart-26f37.png[]
+
[TIP]
====
...
image::dataweave/dataweave-quickstart-4a1db.png[]
====
...
+
image::dataweave/dataweave-quickstart-6d4b7.png[]
...
+
image::dataweave/dataweave-quickstart-3c9c7.png[]
...
image::dataweave/dataweave-quickstart-f39a5.png[]
====
...
+
image::dataweave/dataweave-quickstart-flows-from-raml.png[]
+
...
image::dataweave/dataweave-quickstart-1984d.png[map]
...
+
image::dataweave/dataweave-quickstart-c1e53.png[]
+
...
+
image::dataweave/dataweave-quickstart-4abf1.png[]
...
See how the expression that evaluates `size` has changed--it uses the xref:dataweave/dataweave-language-introduction.adoc#unless-otherwise[Unless and Otherwise operators] to set the value to `M` _unless_ the buyer's state is Texas, in which case the shirt size, expressed as `size` becomes `XXL`.
...
+
image::dataweave/dataweave-quickstart-07f8a.png[]
+
...
+
image::dataweave/dataweave-quickstart-59e9c.png[]
+
...
+
image::dataweave/dataweave-quickstart-75ca5.png[]
+
...
+
image::dataweave/dataweave-quickstart-e4466.png[]
...
+
image::dataweave/dataweave-quickstart-2196b.png[]
...
+
image::dataweave/dataweave-quickstart-be7fd.png[]
...
image::dataweave/dataweave-quickstart-7c173.png[]
...
image::dataweave/dataweave-quickstart-fb3b7.png[]

You can easily change the xref:dataweave/dataweave-language-introduction.adoc#output-directive[output directive] from the default `application/java` to `text/csv` or to any other of the xref:dataweave/dataweave-formats.adoc[supported formats] and see the results of that transformation.

image::dataweave/dataweave-quickstart-c8b4f.png[]
...
Change the xref:dataweave/dataweave-language-introduction.adoc#output-directive[output directive] to `application/json` so that the final result is identical to the sample data in the input. Later on you can transform this in more interesting ways.

image::dataweave/dataweave-quickstart-6381c.png[]
...
As the input is a collection of elements, each describing a product in inventory, this code uses the xref:dataweave/dataweave-operators.adoc#map[map operator] to treat each of these instances separately. What's enclosed in curly brackets right after the map operator is executed once for each element in the collection. Within this operation, the code refers to the currently selected instance via the `$` selector. The code in this example merely takes each item of the input and maps it to an identical field, with the exception of the *size* field, on which it applies the xref:dataweave/dataweave-operators.adoc#upper[upper operator] to change it to upper case.
...
The result from the previous step includes a products for which there is zero stock. We can easily filter those cases out using the xref:dataweave/dataweave-operators.adoc#filter[Filter operator]. Modify your DataWeave code to include `filter $.count > 0` in the end. Your code should look like this:
...
[TIP]
See the xref:dataweave/dataweave-language-introduction.adoc#precedence-table[Precedence Table] to have a clear idea about when each expression is compiled.
...
* More advanced examples in xref:dataweave/dataweave-examples.adoc[DataWeave Examples]
...
* Migrate your old DataMapper transforms automatically by using the xref:dataweave/dataweave-migrator.adoc[DataWeave Migrator Tool]