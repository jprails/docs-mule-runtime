...
You can use these regular expressions in a DW xref:dataweave/dataweave-types.adoc#functions-and-lambdas[function] that first evaluates a string to see which regular expression it matches, and then converts it through the the corresponding function. This function should do the following:

. Take the input string as argument
. Match it with regular expressions in order to determine which format it's in
. Run the xref:dataweave/dataweave-core-functions.adoc#coerce-to-date[coerce to date] function with the corresponding properties
...
* xref:dataweave/dataweave-types.adoc[DataWeave Value Types]
