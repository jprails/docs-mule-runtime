...
:page-aliases: dataweave-streaming.adoc
...
In order for DataWeave to process the file stream, the input must enter DataWeave as a stream. For example, flows that start with a xref:transports/file-transport-reference.adoc[file transport inbound endpoint] must have streaming enabled.
...
image::dataweave/file-to-file-dw-stream.png[file_to_file]
...
image::dataweave/file-to-file-dw-log.png[log DW output]
...
image::dataweave/file-to-file-dw-log-string.png[log DW output as String]
