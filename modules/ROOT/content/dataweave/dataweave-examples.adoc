...
:page-aliases: dataweave-examples.adoc
:keywords: studio, anypoint, transform, transformer, format, aggregate, rename, split, filter convert, xml, json, csv, pojo, java object, metadata, dataweave, data weave, datamapper, dwl, dfl, dw, output structure, input structure, map, mapping

== Introduction

Below are a series of examples that demonstrate various data transformation approaches expressed in xref:dataweave/dataweave-language-introduction.adoc[DataWeave]. DataWeave code is typically written inside the *Transform Message* component, which is accessible in Anypoint Studio.

[NOTE]
To step through basic DataWeave use cases and exercises in actual Mule applications, see the xref:dataweave/dataweave-quickstart.adoc[DataWeave Quickstart Guide].
...
[TIP]
The xref:dataweave/dataweave-quickstart.adoc[DataWeave Quickstart Guide] walks you this same transformation, showing you how you can create it automatically via the drag-and-drop UI. It then goes into tweaking the code to make it more interesting.
...
Follow along to see how to use the xref:dataweave/dataweave-operators.adoc#map[`map`] operator. The `as` operator is also featured in this example. Take an XML list of books and use DataWeave to output a JSON version of the book list. It goes through each `<book>` element from the XML input, simply using the `map` operator.

You can ensure the transformation generates the correct type for each element by using the `as` operator (used for type coercion - see an xref:dataweave/dataweave-operators.adoc#coerce-to-number[`as` operator example]).
...
In this example, the input contains sensitive information that should be removed. The transform replicates the inbound structure but uses a simple xref:dataweave/dataweave-operators.adoc#remove[`remove`] operator to take away specific key:value pairs.

The example goes through the whole set of elements in the input using the xref:dataweave/dataweave-operators.adoc#map[`map`] operator.
...
In this example, the input contains the same sensitive information as in the previous one, but instead of entirely removing the key:value pairs that contain it, the values are replaced with the string. The transform replicates the inbound structure but uses a simple xref:dataweave/dataweave-language-introduction.adoc#when-otherwise[`when`] operator to replace values when specific keys occur.
...
The xref:dataweave/dataweave-operators.adoc#map[`map`] operator carries out the same steps for each element in the input array. Through the use of @, attributes are injected into the XML.

[TIP]
See xref:dataweave/dataweave-selectors.adoc#attribute-selector-expressions[Attribute selector expressions] on how to select XML attributes in DataWeave.
...
The example goes through the whole set of elements in the input using a xref:dataweave/dataweave-operators.adoc#map[`map`] operator. It also uses the xref:dataweave/dataweave-language-introduction.adoc#when-otherwise[`when`] operator to only select instances that include the field 'insurance'.
...
The example goes through the whole set of elements in the input using  a xref:dataweave/dataweave-operators.adoc#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-language-introduction.adoc#when-otherwise[`when`] operator together with an xref:dataweave/dataweave-operators.adoc#and[`and`] to pick out the specific keys that need to change, and an xref:dataweave/dataweave-operators.adoc#coerce-to-number[example use] of the `as` operator to coerce its type to string.
...
This example takes an XML input and parses it into a different XML arrangement. After a single `<header>` element is copied, a xref:dataweave/dataweave-operators.adoc#map[`map` operation] carries out the same steps for each 'item': several fields are passed on without any changes, then the discount and subtotal fields are calculated with references to constants defined in the xref:dataweave/dataweave-language-introduction.adoc#the-dataweave-header[header directives] of the transform. A single set of subtotal, tax and total elements are created by performing a xref:dataweave/dataweave-operators.adoc#reduce[*reduce*] operation over all of the items in the "items" array, performing calculations that sometimes involve constants defined in the header. The xref:dataweave/dataweave-operators.adoc#as[`as`] operator is also used to coerce to a number and then performs xref:dataweave/dataweave-operators.adoc#basic-math-operations[basic math] on these numbers.
...
This example takes in an XML file that is grouped separating two types of elements: teachers and students, the transform uses the xref:dataweave/dataweave-operators.adoc#group-by[*Group By* operation] to make it into an XML where the top level grouping is "class". Within each class, the students and teachers that are registered in it are listed.

The example goes through the whole set of elements in the input using both a xref:dataweave/dataweave-operators.adoc#map-object[*map object* operation] and a xref:dataweave/dataweave-operators.adoc#map[*map* operation]. Note that they differ in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-operators.adoc#filter[*filter* operation] to only show the attendees that have the class subject listed as a hobby. As each student may have multiple hobbies, the xref:dataweave/dataweave-selectors.adoc#multi-value-selector[*Multi value* selector] is used to refer to them all.
...
In this example, there are three different input JSON files, these three all arrive in one single Mule message, occupying the payload and two xref:dataweave/dataweave-selectors.adoc#flow-variables-from-a-mule-message[flow variables]. The payload contains an array of book objects, one flow variable has a set of currency exchange rates, and the other one a query. The transform filters the first input using the conditions passed in the third input, then performs a xref:dataweave/dataweave-operators.adoc#map[`map`] to deal with each remaining object separately. Within this map, it defines two variables: *it* and *props*. Through the use of @, attributes are added into the XML tags. A second map operation inside the first one calculates the price of each book for each of the currencies provided in the second input. Another map operation displays each element in the `author` array as a separate <author></author> tag.


[TIP]
See xref:dataweave/dataweave-selectors.adoc#flow-variables-from-a-mule-message[flow variables] for how to access flow variables from the input and xref:dataweave/dataweave-selectors.adoc#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
The example defines a xref:dataweave/dataweave-types.adoc#functions-and-lambdas[function] in the xref:dataweave/dataweave-language-introduction.adoc#dataweave-header[dataweave header] and then calls it on the body. It goes through the whole set of elements in the input using  a xref:dataweave/dataweave-operators.adoc#map-object[`mapObject`] operator. Note that it differs from the map operation in that map object processes both key and value of its input, rather than just the keys. It also uses the xref:dataweave/dataweave-language-introduction.adoc#when-otherwise[`when` and `otherwise`] operators to pick out only instances where an XML attribute exists, and to do nothing otherwise.
It also uses the xref:dataweave/dataweave-operators.adoc#match[`match` ] operator.

See xref:dataweave/dataweave-selectors.adoc#attribute-selector-expressions[Attribute selector expressions] for how to access XML attributes in DataWeave.
...
When producing an output in xref:dataweave/dataweave-formats.adoc#xml[XML], DataWeave by default outputs every value as enclosed between an opening and a closing tag:
...
[TIP]
See more XML reader and writer properties on xref:dataweave/dataweave-formats.adoc#xml[DataWeave formats].


== Configure CSV Reader

DataWeave supports CSV inputs and outputs, by configuring the reader and writer properties, you can adapt it to different conventions regarding separations, line breaks, etc. Other formats also allow for this kind of configuration. See xref:6@studio::transform-message/input-output-structure-transformation-studio-task.adoc[To Define Input and Output Structure of a Transformation] for how to do this on the UI, or look at the xref:dataweave/dataweave-xml-reference.adoc#reader-configuration[DataWeave XML Reference] for the required XML syntax. You can also see a full reference of the reader properties available with CSV format on xref:dataweave/dataweave-formats.adoc#csv[DataWeave formats].

Reader properties are set as part of the XML `dw:transform-message` component, whilst writer properties are specified in the DataWeave code itself, as part of the output directive. You can see a full reference to the writer properties available with CSV format on xref:dataweave/dataweave-formats.adoc#writer-properties[DataWeave formats].
...
To do this, you can normalize the input with the xref:dataweave/dataweave-operators.adoc#replace[replace] operator so that they all match a single pattern, and then turn the resulting string to a date with xref:dataweave/dataweave-operators.adoc#coerce-to-date[coerce to date].

To improve the clarity in your code, you can easily abstract the normalizing of your string into custom a xref:dataweave/dataweave-types.adoc#functions-and-lambdas[function], declaring it only once on your DW header, and then using it as many times as necessary by simply referencing this function.
...
You can use these regular expressions in a DW xref:dataweave/dataweave-types.adoc#functions-and-lambdas[function] that first evaluates a string to see which regular expression it matches, and then converts it through the the corresponding operation. This function should do the following:

. Take the input string as argument
. Match it with regular expressions in order to determine which format it's in
. Run the xref:dataweave/dataweave-operators.adoc#coerce-to-date[coerce to date] operator with the corresponding properties
...
The following example makes all "keys" lower case. The xref:dataweave/dataweave-language-introduction.adoc#the-dataweave-header[DataWeave header] defines a xref:dataweave/dataweave-types.adoc#functions-and-lambdas[function] that receives another function as an argument and then calls that function in the body, applying itself to the keys and not the values it processes. The function that is sent as an argument is a simple one that uses the xref:dataweave/dataweave-operators.adoc#lower[`lower`] operator to set the specified text to lower case.

The example goes through the whole set of elements in the input using a xref:dataweave/dataweave-operators.adoc#map-object[`mapObject`] operator. It also uses the xref:dataweave/dataweave-operators.adoc#dataweave-language-introduction#when-otherwise[`when` and `otherwise`] operators to pick out only instances _when_ an XML attribute exists, and do nothing _otherwise_.
It also uses the xref:dataweave/dataweave-operators.adoc#match[`match` ] operator.
...
See xref:dataweave/dataweave-selectors.adoc#attribute-selector-expressions[Attribute selector expressions] on how to access XML attributes in DataWeave.
...
* The whole set of elements in the input is mapped using a xref:dataweave/dataweave-operators.adoc#map[`map`] operator.
* The example also uses the xref:dataweave/dataweave-language-introduction.adoc#default[`default`] operator.


[NOTE]
Note that for this example to work, on the payload input you must set the reader configuration so that the 'nullValueOn' attribute is "empty". See xref:6@studio::transform-message/input-output-structure-transformation-studio-task.adoc[To Define Input and Output Structure of a Transformation].
...
* The whole set of elements in the input is mapped using a xref:dataweave/dataweave-operators.adoc#map[`map`] operator.
* This example uses the xref:dataweave/dataweave-operators.adoc#reduce[`reduce`] operator.
* This example also uses the xref:dataweave/dataweave-language-introduction.adoc#when-otherwise[`when`] operator to conditionally display the last field.
* This example also uses the xref:dataweave/dataweave-operators.adoc#split-by[`splitBy`] operator to parse the input.
* Finally this example takes advantage of the ability to define a xref:dataweave/dataweave-types.adoc#functions-and-lambdas[`function`] for re-use.
