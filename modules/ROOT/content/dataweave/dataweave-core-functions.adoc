...
In xref:dataweave/index.adoc[DataWeave] you can carry out many different operations on the elements of a DataWeave transform. This document serves as a reference for all of the core functions in the DataWeave language. See xref:dataweave/dataweave-types.adoc#dataweave-operators-sorted-by-type[all core functions sorted by type]
...
=== now(): DateTime
Returns a xref:dataweave/dataweave-types.adoc#dates[(Datetime)] object with the current date and time.
...
The first argument points to the element that must be written, the second is the format in which to write it. A third optional argument lists writer configuration properties. See xref:dataweave/dataweave-language-introduction.adoc#output-directive[Output Directive] and its sub-sections for a full list of available configuration options for each different format.
...
* For an index of all available functions, categorized by the types you need to supply as parameters, see xref:dataweave/dataweave-types.adoc#operators-sorted-by-type[DataWeave Core Functions Sorted by Type].
