...
If you're deploying multiple applications to the same place and those applications share the same resources, you can create a common *domain* where you define common configurations that can be referenced by multiple projects. This allows you to, for example, expose different services in different projects through the same HTTP host and port and be able to deploy everything without any conflicts. See xref:shared-resources/domain-project-shared-resources.adoc[Shared Resources].
