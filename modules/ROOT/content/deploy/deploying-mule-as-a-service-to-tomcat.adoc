...
For more information on hot deploying Mule applications, see xref:3.7@deploy/application-server-based-hot-deployment.adoc[Application Server Based Hot Deployment].
...
. In the Tomcat `conf/catalina.properites` file, add the following to `common.loader` (preceded by a comma to separate it from existing values:)
+
...
. Package your Mule application's configuration file and custom Java classes in `.war` file (see xref:3.7@deploy/application-server-based-hot-deployment.adoc[Application Server Based Hot Deployment]).
...
* Learn about deploying Mule application in the cloud with xref:cloudhub::index.adoc[CloudHub].