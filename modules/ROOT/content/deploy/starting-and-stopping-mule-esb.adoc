...
:page-aliases: starting-and-stopping-mule-esb.adoc
...
a|`-D-Mmule.mmc.bind.port`|Specify a port or port range for the Mule ESB agent listener that the Management Console binds to. To specify a port range, use `<port>-<port>`, for example `3000-3010`.
|===
...
* *NEXT STEP:* Graduate to the content of the First Week with Mule chapter, starting with xref:3.6@flows/index.adoc[Mule Application Architecture].