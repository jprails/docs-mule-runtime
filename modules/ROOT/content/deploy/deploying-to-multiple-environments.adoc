...
:page-aliases: deploying-to-multiple-environments.adoc
...
link:{attachmentsdir}/connect_with_SFDC.zip[Download Example Studio Application]

[TIP]
====
Should you need to encrypt passwords in your properties file, consider using the xref:security/mule-credentials-vault.adoc[Mule Credentials Vault]. Refer to the documentation for xref:security/anypoint-enterprise-security.adoc[Anypoint Enterprise Security] for more information on securing applications in Mule.
====

== Assumptions

This document assumes that you are familiar with the exercise of building and deploying Mule applications in Mule Studio. Consider reading the xref:general:getting-started:index.adoc[Anypoint Fundamentals] for more information.
...
+
image::deploy/properties-files.png[properties_files]
...
+
image::deploy/properties.png[]
+
[TIP]
====
Learn how to encrypt a properties file using the xref:security/mule-credentials-vault.adoc[Mule Credentials Vault].
====
...
. To configure Studio to use a specific environment when  *deploying on-premise* , *debugging*  or  *using xref:5@studio::datasense/datasense.adoc[DataSense]*  to retrieve metadata from a SaaS provider, double-click to open your application's  `mule-project.xml` file, located in the root directory of your project.
+
[WARNING]
====
...
+
image::deploy/runtime-env.png[runtime_env]
...
+
image::deploy/deploy-cloudhub.png[deploy_cloudhub]
...
* Learn more about encrypting a properties file using the xref:security/mule-credentials-vault.adoc[Mule Credentials Vault].

* To learn more about deploying to test environments in CloudHub, access the CloudHub Sandbox Environments documentation.

* Learn more about xref:configuring-properties.adoc[Properties Placeholders] in Mule.

* Learn how to proceed when deploying multiple applications with xref:shared-resources/domain-project-shared-resources.adoc[Shared Resources]