...
:page-aliases: embedding-mule-in-a-java-application-or-webapp.adoc
...
[NOTE]
Before running Mule in embedded in mode in either a Java application or Web app, be sure to manually update all the required dependencies for your application. The Mule ESB Embedded Single .jar contains all Mule classes, but the dependencies must be included manually.
...
Access the xref:deploy/embedding-mule-in-a-java-application-or-webapp.adoc[Bookstore example application] to witness Mule as an embedded as a web application inside a Servlet container.

== Interacting with Mule from Your Code

To interact with the Mule server from your application, JSP, or servlet, you can use the xref:3.6@administer/using-the-mule-client.adoc[Mule Client]
