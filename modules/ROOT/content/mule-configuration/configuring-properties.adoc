...
:page-aliases: configuring-properties.adoc
...
You can use http://ant.apache.org/[Ant]-style property placeholders in your Mule ESB configuration. For example:
...
[WARNING]
====
If you deploy multiple applications through a xref:shared-resources/domain-project-shared-resources.adoc[Shared Resources] structure, don't set anything in the properties files, as there might potentially be conflicts between the various apps that share a domain. Instead, set environment variables over the scope of the deployed app, its domain, and other apps under that domain.

As explained in xref:shared-resources/domain-project-shared-resources.adoc[Shared Resources], in Studio you can create these variables through the *Environment tab* of the *Run Configurations menu*, reachable via the drop-down menu next to the Play button.
====

In Anypoint Studio, you can create and edit a properties file in your project's folders.

image::mule-configuration/properties-file.png[properties+file]
...
https://www.mulesoft.org/docs/site/3.6.0/apidocs/org/mule/config/DefaultMuleConfiguration.html[org.mule.config.DefaultMuleConfiguration]

For information on configuration builders, see xref:mule-configuration/about-configuration-builders.adoc[About Configuration Builders].
...
+
image::mule-configuration/arguments-tab.png[Arguments+Tab]
+
...
+
image::mule-configuration/environment-tab.png[Environment+Tab]
...
image::mule-configuration/environment-tab2.png[Environment+Tab2]

== Setting Properties Values in CloudHub

If you deploy your application to xref:cloudhub::index.adoc[CloudHub], you can also set properties through the Runtime Manager console. These can be defined when xref:runtime-manager::deploying-to-cloudhub.adoc[Deploying to CloudHub], or on an xref:cloudhub::managing-applications-on-cloudhub.adoc[already running application].
...
. Log in to your https://anypoint.mulesoft.com/#/signin[Anypoint Platform] account.
. Click *CloudHub*.
...
See xref:cloudhub::managing-applications-on-cloudhub.adoc[Managing Applications on CloudHub] and xref:runtime-manager::secure-application-properties.adoc[Secure Application Properties] for more details.
