...
*Required*: no +
*Default*: `true`
|`flowEndingWithOneWayEndpointReturnsNull` |Backwards Compatabilty Flag: Used to revert to Mule 3.2 behavior of one-way outbound endpoints when they are the last element inside a request-response flow. When true, the value returned by the endpoint is null, otherwise it's an echo of the received event. +
*Type*: `boolean` +
*Required*: no +
*Default*: `false`
