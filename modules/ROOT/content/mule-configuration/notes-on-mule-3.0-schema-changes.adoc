...
* The `synchronous` attribute has been removed in favor of the xref:configuring-endpoints.adoc[exchange-pattern] attribute.
* xref:configuring-endpoints.adoc[Dynamic endpoints] can be designated on most transports.
* Transformers, routers, filters, and other constructs that affect the processing of messages are now all xref:message-sources-and-message-processors.adoc[Message Processors] and can be used more flexibly inside a Mule configuration.
* In addition to services, message processing can be configured using xref:flows/using-flows-for-service-orchestration.adoc[flows] and xref:using-mule-configuration-patterns.adoc[configuration patterns].

== BPM

* xref:modules/bpm-module-reference.adoc[BPM] now includes the element `jbpm-connector` to specify a jBpm connector.

== CXF

* The CXF schema is quite different now that CXF has been changed from a transport to a module. For details, see xref:upgrading-cxf-from-mule-2.adoc[Upgrading CXF from Mule 2].
