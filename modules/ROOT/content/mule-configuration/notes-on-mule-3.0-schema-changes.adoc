...
* In addition to services, message processing can be configured using xref:3.6@flows/using-flows-for-service-orchestration.adoc[flows] and xref:configuration-patterns.adoc[configuration patterns].
...
* The e-mail connectors (xref:transports/imap-transport-reference.adoc[IMAP], xref:3.6@transports/imap-transport-reference.adoc[IMAPS], xref:transports/pop3-transport-reference.adoc[POP3], and xref:3.7@transports/pop3-transport-reference.adoc[POP3S]) support a new attribute `moveToFolder`, which specifies a folder to move e-mail messages to after they have been read.
...
* The xref:modules/xml-module-reference.adoc[XML] now includes JAXB and xref:3.7@modules/xquery-support.adoc[XQuery Support] transformers and extensions.
