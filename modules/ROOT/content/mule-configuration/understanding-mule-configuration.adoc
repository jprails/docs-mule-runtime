...
[WARNING]
The following description of expressions refers to the pre-Mule 3.3 expression evaluators. For a current reference to using expressions in Mule, see xref:mel/mule-expression-language-mel.adoc[Mule Expression Language MEL].
...
❷ is illegal. The address of an inbound endpoint must be set at configuration time, and the expression cannot be evaluated before the configuration is built. +
❸ is precisely like ❶. +
...
❾ calls authorize.net to authorize a credit card purchase, passing it information from the message.  For more about connectors, see xref:3.3@connectors::index.adoc[Anypoint Connectors].
