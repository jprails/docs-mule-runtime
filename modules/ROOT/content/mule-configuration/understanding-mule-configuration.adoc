...
For more about Mule transformers, see xref:3.6@transformers/using-transformers.adoc[Using Transformers].
...
For a current reference to using expressions in Mule, see xref:3.7@mel/mule-expression-language-mel.adoc[Mule Expression Language MEL].
...
Message enrichment is done using the `enricher` element. Unlike message transformation, which alters the current message's payload, enrichment adds additional properties to the message. This allows the flow to build up a collection of information for later processing.  For more about enriching messages see xref:3.6@scopes/message-enricher.adoc[Message Enricher].
...
The `logger` element allows debugging information to be written from the flow.  For more about the logger see xref:3.6@components/logger-component-reference.adoc[Logger Component Reference]
...
A router changes the flow of the message. Among other possibilities, it might choose among different message processors, split one message into many, join many messages into one.  For more about routers, see xref:3.6@routers/routers.adoc[Routing Message Processors].
...
❶❷ This is a Java component, specified by its class name, which is called with the current message. In this case, it preprocesses the message.  For more about entry point resolvers, see xref:3.7@components/entry-point-resolver-configuration-reference.adoc[Entry Point Resolver Configuration Reference].
...
The starred transaction managers allow you to configure a JNDI environment before performing the lookup.  For more about transaction managers, see xref:3.6@transactions/transaction-management.adoc[Transaction Management].
...
For more about Mule security, see xref:3.7@security/configuring-security.adoc[Configuring Security].
...
For more about notifications, see xref:3.6@mule-configuration/notifications-configuration-reference.adoc[Notifications Configuration Reference].

=== Agents

Mule allows you to define Agents to extend the functionality of Mule. Mule will manage the agents' lifecycle (initialize them and start them on startup, and stop them and dispose of them on sutdown). These agents can do virtually anything; the only requirement is that they implement `org.mule.api.agent.Agent`, which allows Mule to manage them.  For more about Mule agents, see xref:3.7@administer/mule-agents.adoc[Mule Agents].
