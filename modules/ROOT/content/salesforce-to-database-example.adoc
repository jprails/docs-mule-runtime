...
image:batch.png[]xref:batch/batch-processing.adoc[Batch Processing] allows you to split a payload into individual elements to process each individually. This functionality is particularly useful when working with streaming input or when engineering "near real-time" data integration between SaaS applications.

== Database Connector

image:icon-database-blue-big-2.png[icon-database-blue-big+(2)] The xref:3.5@connectors::database-connector.adoc[Database Connector] provides a standardized way to access to any JDBC relational database, consistently using one same interface for every case. This connector allows you to run diverse SQL operations on your database, including Select, Insert, Update, Delete, and even Stored Procedures.

== Message Enriching

image:enriched4.png[enriched4+(1)]Mule uses xref:scopes/message-enricher.adoc[Message Enrichers] to enrich message payloads with data (i.e. add to the payload), rather than changing payload contents. Mule enriches a message’s payload so that other message processors in the application can access the original payload.
...
Unlike typical Mule projects that are organized into Flows, this project runs a xref:batch/batch-processing.adoc[Batch Process]. The process is divided into three stages where actions have different scopes:
...
* Understand xref:batch/batch-processing.adoc[Batch Processing].
* Learn more about the xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Datamapper].
* Learn about xref:3.5@connectors::index.adoc[Anypoint Connectors].
* Understand the xref:scopes/poll-reference.adoc[Poll Scope].
* Read more about the xref:routers/choice-flow-control-reference.adoc[Choice Flow Control].
* Read more about the xref:scopes/message-enricher.adoc[Message Enricher].
* Learn more about xref:5@studio::datasense/datasense-query-language.adoc[DataSense Query Language] to write queries in Mule connectors which support DSQL.