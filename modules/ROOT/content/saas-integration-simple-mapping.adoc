...
If you were tempted to answer "b" or "c" to any of the questions above, we highly recommend that you take four minutes to read *xref:general:getting-started:index.adoc[Anypoint Fundamentals]* before getting started with this Shortcut to Success.
...
We added a *xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper Transformer]* to our flow and told it the type of input data it can expect (flat file), the type of output data we want it to produce (structured), then used the drag-and-drop interface to visually map input fields to output fields. In its lovely completed form, DataMapper gives us visibility into all these transformation details and visually represents each mapping with arrows.
...
The Salesforce connector also requires a xref:global-elements.adoc[Global Element], which contains global configurations. At minimum, the global configuration should include a Username, Password, and Security Token, shown in the orange box (below). (You can configure plenty more optional details if you want: http://mulesoft.github.io/salesforce-connector/mule/sfdc.html[knock yourself out].)
...
. Add a xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper] transformer into your flow in between your message source and your connector.
+
...
. Configure the DataMapper transformer. If your connector is DataSense-enabled, you should only need to configure the input values. Otherwise, you can quickly map inputs to outputs by clicking and dragging input fields to output fields in the xref:5@studio::datamapper/datamapper-concepts.adoc[Data Mapping Console].
...
* Try out more complex xref:5@studio::choosing-mel-or-ctl2-as-scripting-engine.adoc[DataMapping using Mule Expression Language] to extract specific data from your message header or payload and dynamically map to your desired format.
