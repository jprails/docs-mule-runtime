...
:page-aliases: securing-a-soap-api.adoc
...
To the Mule flow that involves an API, add a xref:3.6@components/cxf-component-reference.adoc[*CXF Component*] to publish, consume, or proxy an API. Complete the following steps to configure WS-security on the CXF component.
...
+
image::api-usage/cxfsecuritytab.png[CXFsecuritytab]
. Click the
image:api-usage/add.png[(plus)] icon in the *Add Configuration Element* pane to create a new key-value pair.
image:api-usage/studio-wssecurity-add.png[Studio-wssecurity-add]
+
[TIP]
...
+
image::api-usage/studio-wssecurity-token.png[Studio-wssecurity-token]
+
[%header,cols="2*"]
|===
...
+
[TIP]
If you have not yet created any beans, click the image:api-usage/add.png[(plus)] button to open a new properties panel in which you can create and configure a new bean. The bean imports the Java class you have built to specify the custom validator's override behavior.
+
image::api-usage/studio-wssecurity-custom.png[Studio-wssecurity-custom]
...
For a complete explanation of the example application, see xref:2.0@exchange::exchange1.adoc[the Library].
...
* Learn more about configuring a xref:3.6@components/cxf-component-reference.adoc[CXF component] in your Mule application.
* See xref:2.0@exchange::exchange1.adoc[examples].