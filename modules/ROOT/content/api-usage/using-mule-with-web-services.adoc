...
* xref:api-usage/proxying-web-services.adoc[Creating a proxy/gateway to existing Web services] (for example, adding security to an existing Web service)
...
For simple use cases, you may consider using the xref:web-service-proxy-pattern.adoc[Web Service Proxy Pattern], particularly when you don't need to accomplish much message processing. From Mule, you can use xref:using-mule-configuration-patterns.adoc[Using Mule Configuration Patterns] to configure (with minor XML configuration) a specific integration feature.
