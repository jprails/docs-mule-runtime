...
Alternatively, to secure a web service running on Mule (where Mule is the server), you can set the https://www.mulesoft.org/docs/site/3.7.0/apidocs/org/mule/module/acegi/filters/http/HttpBasicAuthenticationFilter.html[HttpBasicAuthenticationFilter] in the web service component. Any call made to the web service has to pass through the filter that delegates the authentication to Acegi.
