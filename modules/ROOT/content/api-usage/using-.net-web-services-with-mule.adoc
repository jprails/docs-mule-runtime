...
:page-aliases: using-.net-web-services-with-mule.adoc
...
Alternatively, to secure a web service running on Mule (where Mule is the server), you can set the https://www.mulesoft.org/docs/site/current/apidocs/org/mule/module/acegi/filters/http/HttpBasicAuthenticationFilter.html[HttpBasicAuthenticationFilter] on the web service component. Any call made to the web service would have to pass through the filter that delegates the authentication to Acegi.

Another alternative would be to use HTTPS where certificates are used for authentication.

For more information see xref:security/configuring-security.adoc[Configuring Security] and #[Using HTTPS with CXF]#.
...
If you are using CXF, you can configure a client and service to use WS-Security. For details, see xref:modules/enabling-ws-security.adoc[Enabling WS-Security].
