...
https://www.mulesoft.org/docs/site/3.8.0/apidocs/org/mule/transport/http/filters/HttpBasicAuthenticationFilter.html[HttpBasicAuthenticationFilter] on the web service component. Any call made to the web service would have to pass through the filter that delegates the authentication to Acegi.

Another alternative would be to use HTTPS where certificates are used for authentication.

For more information see xref:security/configuring-security.adoc[Configuring Security].
