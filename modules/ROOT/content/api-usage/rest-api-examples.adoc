...
image::api-usage/rest-api-examples-ec226.png[]
...
image::api-usage/rest-api-examples-bf47c.png[]

In *Connector Configuration*, click image:api-usage/add.png[], and set up the *Host* and *Port*:

image::api-usage/rest-api-examples-876de.png[]
...
. In *Connector Configuration*, click image:api-usage/add.png[], and set up the *Host*, *Port*, and *Base Path*:
+
image::api-usage/rest-api-examples-fc78a.png[]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
|===
+
image::api-usage/rest-api-examples-e5e5f.png[]
+
...
+
image::api-usage/rest-api-examples-ba00e.png[]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
+
image::api-usage/rest-api-examples-d0064.png[]
+
. In *Connector Configuration*, click image:api-usage/add.png[], and enter a path in *Write to Directory*. Enter `/tmp` for example.
+
image::api-usage/rest-api-examples-479e3.png[]
...
+
image::api-usage/rest-api-examples-4c1b4.png[]
+
...
image::api-usage/rest-api-examples-1cd70.png[]

The first element, an xref:3.8@connectors::http-connector.adoc[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Calling the listener triggers the flow. Requests to the HTTP listener must take the form:
...
The HTTP request connector passes the XML it receives from the API to a byte-array-to-string transformer, which converts the message payload to a string. Next, a xref:routers/splitter-flow-control-reference.adoc[splitter] splits the message into parts. The splitter is configured to split the incoming message using the  `<quote`> element in the XML as delimiter.
...
The last component in the flow, a xref:3.8@connectors::database-connector.adoc[Database (JDBC)] connector, receives each XML chunk corresponding to each queried symbol. The JDBC connector is configured to run the following SQL query:
...
+
image::api-usage/rest-api-examples-2979f.png[]
+
. In *Connector Configuration*, click image:api-usage/add.png[], and set up the *Host* and *Port*:
+
image::api-usage/rest-api-examples-c6d2d.png[]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
. Select the HTTP Request Connector, and in the properties editor click image:api-usage/add.png[] to create a new Connector Configuration Element.
. Set up the *Host* and *Base Path*:
+
image::api-usage/rest-api-examples-426f6.png[]
+
[%header,cols="20a,80a"]
|===
|Parameter |Value
...
. In the properties editor, set up the *Path* and *Method*:
+
image::api-usage/rest-api-examples-4ec20.png[]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
The *Splitter* divides the incoming message into parts based on a user-defined XPATH expression. The expression conforms to the xref:3.6@modules/xpath.adoc[XPath syntax supported by MEL]:
...
----
+
image::api-usage/rest-api-examples-3756b.png[]
+
...
+
image::api-usage/rest-api-examples-51c06.png[]
+
*General Tab*
[%header%autowidth.spread]
|===
...
. Install the correct xref:3.8@connectors::database-connector.adoc[database driver] for your database in your Studio application.
.  xref:3.8@connectors::database-connector.adoc#adding-the-database-driver-for-generic-db-configuration[Add the Postgres driver to the build path.]
. Start Postgres.
...
. In *Connector Configuration*, click image:api-usage/add.png[].
+
The *Choose Global Type* dialog appears.
+
image::api-usage/rest-api-examples-deccf.png[]
+
...
+
image::api-usage/rest-api-examples-82a84.png[]
+
...
----
+
image::api-usage/rest-api-examples-3678e.png[]
...
+
image::api-usage/rest-api-examples-b8dd8.png[]
