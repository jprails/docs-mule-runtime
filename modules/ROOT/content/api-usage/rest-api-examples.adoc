...
:page-aliases: rest-api-examples.adoc
...
image::api-usage/rest-api-1.png[rest+api+1]
...
The first element, an xref:3.6@connectors::http-connector.adoc[HTTP Inbound Endpoint], listens on localhost port 8081 (the default) for incoming GET requests. Hitting the listener triggers the flow. Requests to the HTTP inbound endpoint must take the form:
...
image::api-usage/http-listener-basic.png[http+listener+basic]
...
image::api-usage/http-config-1.png[http+config+1]
...
+
image::api-usage/http-global-element-baconipsum.png[http+global+element+baconipsum]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
+
image::api-usage/http-connector-basic.png[http+connector+basic]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
+
image::api-usage/request-connector-w-params.png[request+connector+w+params]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
image::api-usage/yahoo-xml.png[yahoo+xml]
...
The first element, an xref:3.6@connectors::http-connector.adoc[HTTP inbound endpoint], listens on localhost port 8081 (the default) for incoming GET requests. Hitting the connector triggers the flow. Requests to the HTTP Inbound Endpoint must take the form:
...
The HTTP Outbound Endpoint passes the XML it received from the API to a byte-array-to-string transformer, which converts the message payload to a string. Next, a xref:3.6@routers/splitter-flow-control-reference.adoc[splitter] splits the message into parts. The splitter is configured to split the incoming message using the XML element `<results`> as delimiter. The reason for this is that the REST API uses `<results>` to delimit the information for each stock symbol retrieved, as shown below.
...
To split the incoming XML, the splitter uses the xref:3.7@mel/mule-expression-language-mel.adoc[Mule Expression Language] expression `#[xpath('//results')]`, which contains the http://en.wikipedia.org/wiki/XPath[XPath] expression that actually retrieves the XML element `<results>`.

The last element in the flow, a xref:3.7@connectors::database-connector.adoc[Database (JDBC)] connector, receives each XML chunk corresponding to each queried symbol. The JDBC connector is configured to run the following SQL query:
...
. Create a new Connector Configuration Element and set up the *Host* and *Base Path*:
+
image::api-usage/yahoo-global-element.png[yahoo+global+element]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
+
image::api-usage/yahoo-http-w-params.png[yahoo+http+w+params]
+
[%header%autowidth.spread]
|===
|Parameter |Value
...
|*Display Name* |`Byte Array to String` .5+|image:api-usage/byte-array-to-str.png[byte_array_to_str]
|*Return Class* |`-`
|*Ignore Bad Input* |no
|*Encoding* |`-`
|*MIME Type* |`-`
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Byte Array to String` .4+|image:api-usage/byte-array-to-str.png[byte_array_to_str]
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .5+|image:api-usage/db-conn-general-tab.png[db_conn_general_tab]
...
image::api-usage/db-conn-adv-tab.png[db_conn_adv_tab]
...
The database global connector is a xref:3.4@global-elements.adoc[global element] that contains the connection configuration for the desired database. It is referenced by the database connector in the application flow.

You must install the appropriate driver for your database in your Studio application. For details, see the Adding the Database Driver section in xref:3.6@connectors::database-connector.adoc[Database Connector].


[%header,cols="33,33,34"]
|===
|Parameter |Value |Configuration window image
|*Name* |`Postgres` .5+|image:api-usage/db-conn-general-tab.png[db_conn_general_tab]
...
image::api-usage/yahoo-flow-2.png[yahoo+flow+2]
...
The HTTP Outbound Endpoint passes the JSON it received from the API to a xref:3.7@modules/json-module-reference.adoc[JSON-to-object transformer] configured to output a java.util.Map object. The transformer sends this object, which contains the JSON data as key=value pairs, to the last element in the flow, a xref:3.6@connectors::database-connector.adoc[database connector]. This connector uses an SQL query with embedded xref:3.7@mel/mule-expression-language-mel.adoc[Mule Expression Language] expressions to extract specific values from the JSON and insert them into the external database. The full SQL query is shown below.
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`JSON to Object` .2+|image:api-usage/json-to-object-general.png[json.to.object.general]
|*Mapper Reference* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Return Class* |`java util.Map` .4+|image:api-usage/json-to-object-adv-tab.png[json.to.object-adv_tab]
|*Ignore Bad Input* |`no`
|*Encoding* |`-`
|*MIME Type* |`-`
|===
...
|===
|Parameter |Value |Configuration window image
|*Display Name* |`Database` .5+|image:api-usage/db-conn-json-gral-tab.png[db_conn_json_gral_tab]
...
|===
|Parameter |Value |Configuration window image
|*Name* |`Postgres` .5+|image:api-usage/db-global-elem.png[db_global_elem]
...
image::api-usage/pgadmin-json.png[]

== See Also

http://developer.yahoo.com/yql/guide/running-chapt.html[Yahoo Query Language]