...
The first element, an xref:3.7@connectors::http-connector.adoc[HTTP listener], listens on localhost port 8081 (the default) for incoming GET requests. Hitting the listener triggers the flow. Requests to the HTTP listener must take the form:
...
Note that you must install the appropriate driver for your database in your Studio application. For details, see the Adding the Database Driver section in xref:3.7@connectors::database-connector.adoc[Database Connector].
...
The HTTP Request Connector passes the JSON it received from the API to a xref:modules/json-module-reference.adoc[JSON-to-object transformer] configured to output a java.util.Map object. The transformer sends this object, which contains the JSON data as key=value pairs, to the last element in the flow, a xref:3.7@connectors::database-connector.adoc[database connector]. This connector uses an SQL query with embedded xref:mel/mule-expression-language-mel.adoc[Mule Expression Language] expressions to extract specific values from the JSON and insert them into the external database. The full SQL query is shown below.
