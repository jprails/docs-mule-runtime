...
:page-aliases: 3.7@mule-runtime::mule-message.adoc, general:getting-started:mule-message.adoc
:keywords: studio, components, elements, message, mule message, architecture, tutorial

image::messages/mulestudioa.png[mulestudio]
...
This tutorial also assumes that you have a basic understanding of the xref:mule-message-structure.adoc[Mule message structure].
...
+
image::messages/basic-flow.png[basic+flow]
+
...
====
+
image::messages/log-output.png[log+output]
+
...
[source,xml,linenums]
----
include::{samplesdir}/mule-message_1.xml[]
----
+
...
+
image::messages/deploy-success.png[]
+
...
+
image::messages/basic-flow-2.png[basic+flow+2]
+
...
+
image::messages/set-var-auto.png[]
+
...
+
image::messages/set-var-exp2.png[]
+
...
+
image::messages/mel-tutorial-set-variable.png[]
+
...
+
image::messages/breakpoints.png[]
+
...
+
image::messages/debugging-flow.png[debugging+flow]
+
...
+
image::messages/debugger-con-globitos.png[debugger+con+globitos]
+
...
+
image::messages/exloring-query-params.png[exloring+query+params]
+
...
. Press F8 or click the Resume icon (image:messages/resume-icon.png[Resume+icon]) to prompt Mule to unpause processing and continue to the next breakpoint, which, in this example, is the Logger. Note that the Variables tab now includes one entry.
+
image::messages/debug-newvariable.png[Debug-newvariable]
+
...
image::messages/debug-var-edit.png[Debug-var-edit]
...
. With your flow paused at your Logger breakpoint, open the Mule Expression Evaluator by clicking the image:messages/x-plus-y.png[x+plus+y] icon. In the yellow box that opens, you can test out any MEL expression. Mule immediately evaluates the expression, relative to the breakpoint in your flow where your message is currently paused, and either returns a value or throws an exception.
...
For more ideas on MEL expressions to test out in the Mule Expression Evaluator, refer to the xref:mel/mule-expression-language-basic-syntax.adoc[syntax guide] or borrow some from the collection of introductory xref:mel/mule-expression-language-examples.adoc[MEL examples].
...
----
+
image::messages/logger-config-mel.png[]
+
...
[source,xml,linenums]
----
include::{samplesdir}/mule-message_2.xml[]
----
...
==== image:messages/hints.png[] *Hints*
...
==== image:messages/answer.png[] Answer
...
+
image::messages/mmt-extra-credit-flow.png[]
+
...
+
image::messages/expression-mel.png[]
...
[source,xml,linenums]
----
include::{samplesdir}/mule-message_3.xml[]
----

== See Also

* *NEXT STEP:* Learn how to xref:deploy/starting-and-stopping-mule-esb.adoc[start and stop Mule] from the command line.
* Refer to the main xref:mel/mule-expression-language-mel.adoc[MEL documentation] including the collection of xref:mel/mule-expression-language-examples.adoc[examples], which demonstrate how to use MEL expressions in a variety of other message processors.
* Access the Mule Expression Language xref:mel/mule-expression-language-reference.adoc[reference] page for a searchable listing of context objects and their fields, operations, and functions.
* Learn more about xref:6@studio::studio-visual-debugger.adoc[Visual Debugger], the xref:components/logger-component-reference.adoc[Logger], and xref:6@studio::datasense/datasense.adoc[DataSense].
...
** xref:general:getting-started:content-based-routing.adoc[Content-Based Routing Tutorial]
** xref:general:getting-started:anypoint-connector.adoc[Anypoint Connector quickstart]