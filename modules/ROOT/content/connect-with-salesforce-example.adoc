...
This document assumes that you are familiar with Mule and the xref:general:getting-started:index.adoc[Anypoint™ Studio interface]. To increase your familiarity with Studio, consider completing one or more xref:5@studio::basic-studio-tutorial.adoc[Anypoint Studio Tutorials]. Further, this example assumes that you have a basic understanding of xref:flows/index.adoc[Mule flows], xref:global-elements.adoc[Mule Global Elements], and Studio's xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[Anypoint DataMapper].
...
The xref:3.5@connectors::file-connector.adoc[File connector]  Polls the input folder for new files every ten seconds. When it spots a new file, it reads it and passes the content to the xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper Transformer]. This transformer not only converts the format of the data from CSV to a collection, it automatically maps the input fields from the CSV file – FirstName, LastName, etc. – to output fields that Salesforce uses in a collection.

Each mapping earns an arrow which helps you to visualize the activity that occurs within the DataMapper transformer. When it has converted all the contacts in the file to a collection of Salesforce-friendly data, the application uses a https://mulesoft.github.io/salesforce-connector/[Salesforce Connector] to push data into your Salesforce account. The connector's configurations specify the *operation* – `Create` – and the *sObject type* – `Contact` – which dictate exactly how the data uploads to Salesforce; in this case, it creates new contacts.

While the application's functionality is relatively straightforward, the beauty of this project is illustrated through its use of xref:5@studio::datasense/datasense.adoc[DataSense]. Rather than building the application serially – adding, then configuring each of the elements manually according to the order in which they appear in the flow– you can use DataSense to complete the most difficult configurations automatically. The following steps outline the process to build this application.
...
Follow xref:5@studio::import-export-projects-studio.adoc[the procedure] to export auto-generated Studio documentation.

== See Also

* Learn more about xref:5@studio::testing-connections.adoc[Connection Testing] and xref:5@studio::datasense/datasense.adoc[DataSense].
