...
For further details on MEL syntax, see the full xref:3.7@mel/mule-expression-language-reference.adoc[Mule Expression Language Reference] material.

== Assumptions

This document assumes that you have read xref:3.7@mel/mule-expression-language-mel.adoc[Mule Expression Language MEL] and have studied the xref:3.7@mule-message-structure.adoc[Mule message structure].
...
The term *Context Object* forms the first part of the simplest form of a MEL expression; the second part is the *Field* (see image below, left). Use a *Variable* in a MEL expression to access information contained within a Flow Variable or Session Variable on your Mule message. Itself a top-level identifier in MEL, a variable does _not_ require that you define a context object in an expression (see image below, right).
                                   ...
                                   image:mel/contextob-field.png[contextob_field]           image:mel/flowvars-syntax.png[flowVars-syntax]
...
For a full list of syntax rules, see full xref:3.7@mel/mule-expression-language-reference.adoc[MEL reference] material. +
...
There is really no such thing as a single _typical_ MEL expression.** **That said, a few example expressions can help illustrate how MEL expressions resolve. As the following table of examples demonstrates, the values that MEL expressions return can be numerical values, logical values (`true` or `false`), strings, or virtually any data type. MEL expressions can also perform operations, invoke methods, and execute functions. Explore all the possibilities by consulting the complete xref:3.6@mel/mule-expression-language-basic-syntax.adoc[syntax reference]. Access full xref:3.7@mel/mule-expression-language-examples.adoc[examples] that illustrate how to use MEL expressions in applications.
...
[TIP]
For the complete reference on date and time functions, see xref:3.6@mel/mule-expression-language-date-and-time-functions.adoc[Mule Expression Language Date and Time Functions].
...
* Access full *xref:3.7@mel/mule-expression-language-examples.adoc[examples]* that illustrate how to use MEL expressions in applications.
* Access a full set of *xref:3.7@mel/mule-expression-language-reference.adoc[reference material]* for MEL. +
* Access a complete list of *xref:3.7@mel/mule-expression-language-tips.adoc[tips and gotchas]* when using MEL.