...
If you aren't yet familiar with how to access information about the Mule messages that pass through your applications, consider following this xref:3.8@messages/mule-message.adoc[tutorial], which walks you through both examining your Mule message and its data structure and writing simple MEL expressions.
...
In this example, like in the previous one, the Mule application saves a CSV file with user data and returns a greeting. The call to the service includes two parameters, `username` and `age`. The service stores these two parameters and adds a third boolean parameter that evaluates if the user is above a certain age (if `age ``> 18`). In this case, however, the mapping between input and output fields and the generation of the third field is performed by a xref:6@studio::datamapper/datamapper-user-guide-and-reference.adoc[DataMapper] component.
...
+
image::mel/mel-ex-datamapper.png[mel+ex+datamapper]
+
...
+
image::mel/mel-ex-datamapper2.png[mel+ex+datamapper2]
+
