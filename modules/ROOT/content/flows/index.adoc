...
* Sometimes, the Mule application can *return an altered or replacement message* to the _source_ of the original message.
...
A typical example of a message source is an xref:3.8@connectors::index.adoc[*Anypoint Connector*].
...
image::flows/message-source-and-queue.png[Message+source+and+queue]
...
Under certain conditions, flows do not need to be triggered by message sources. For instance, a xref:flows/flow-reference-component-reference.adoc[Flow Reference Component] can trigger a different flow in the same application. Similarly, the xref:scopes/async-scope-reference.adoc[Async Scope] can trigger a another flow that executes asynchronously, (that is, in parallel with the originating flow).
...
image::flows/request-response-endpoints.png[request+response+endpoints]
...
image::flows/processing-strategies.png[processing+strategies]

For further details, see xref:flows/flow-processing-strategies.adoc[Flow Processing Strategies].
...
image::flows/catalog-flow-schematic-2.png[]
...
image::flows/mule-application-architecture.png[mule+application+architecture]

[source,xml,linenums]
----
include::{samplesdir}/mule-application-architecture.xml[]
----

== Flow Configuration

Although flows are very flexible, you cannot place just _any_ processor at _any_ position within a flow. The proximity or absence of certain processors within a sequence can determine whether a given processor can be placed at a certain point within a flow. Finally, depending where it resides in a flow, a given processor, especially an endpoint, can expose an significantly different set of attributes for configuration.
...
* Call various other xref:flows/flows-and-subflows.adoc[flows]

== See Also

* *NEXT STEP:* Learn about how xref:flows/flows-and-subflows.adoc[flows and subflows] work together in an application.
* Examine an xref:flows/flow-architecture-advanced-use-case.adoc[advanced use case] showing a more complex flow architecture.